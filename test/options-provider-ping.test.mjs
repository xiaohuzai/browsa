...
test('options.js: init() renders the two fixed agent cards plus a reserved empty LLM slot', () => {
  const cards = providerCards();
  assert.equal(cards.length, 3, 'Hermes Agent + OpenSquilla + one reserved empty LLM slot');
...
test('options.js: configuring + saving the reserved slot does NOT auto-create an extra empty card', async () => {
  ...
  assert.deepEqual(names, ['Hermes Agent', 'OpenSquilla', 'My OpenAI'], 'only the configured provider renders — no auto-appearing empty card');
  ...
  assert.deepEqual(after, ['Hermes Agent', 'OpenSquilla', 'My OpenAI', 'LLM 2'], 'Add appends below the configured provider');
