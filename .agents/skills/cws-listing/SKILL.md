...
2. **两轮因「产品说明关键词过多」被拒（Yellow Argon），判定线是「正文里多个第三方产品名并列」**：R1（0.35.1）是站点清单；R2（0.39.0 首提）是裸扩展名 `.docx / .pptx / …`；R3（0.39.0-r2 重提）连**产品名清单** `Word、Excel、PPT、ePub 等` 和工具名括号 `（Codex、Claude Code、pi）` 都被点名——后者还是基线里活了几个版本的行，说明审核是逐轮收紧、worst-first 打地鼠。**安全形态（两轮均未点名，照用）**：产品名单独出现（`Ollama 等本地运行时`、`自建服务（如 Hermes）`、`GitHub 文件页`、`OpenAI 兼容端点`）；功能动词清单（`总结/翻译/解释/大纲`）；快捷键清单；名称字段 `· ChatGPT · Claude · Ollama`（75 字符内兼容声明惯例，**别主动去动它**）。**必改形态**：任何 2+ 第三方产品名并列（顿号/逗号/括号）一律拆成通用品名词（数学公式、流程图、数据图表；协议不点名）。**改一处必须两语全文扫同款模式一次改到位**（EN/zh 详细说明 + 两语 What's new）——上轮只改被点名行，下轮就点你没改的那行；被拒重提时其余字段不动、不删件。
