...
想问的是一段文字？划选它，用**浮动工具栏**或**右键菜单**（提问 / 解释 / 翻译 / 总结）——不用点 📎。
...
| **Mermaid · ECharts · Markmap** | ` ```mermaid ` / ` ```echarts ` / ` ```markmap ` 代码块内联渲染，各带缩放/复制/导出 SVG 工具栏；直接说要一张图表或思维导图——模型懂这个格式 |
...
| **浮动工具栏与右键菜单** | 在任意页面划选文字：提问 · 解释 · → 中文 · 总结；或右键 → browsa › 提问 / 解释 / 翻译 / 总结 |

</details>

<details>
<summary><b>设置</b>——域名规则、脱敏、长附件自动总结……</summary>

| 设置 | 说明 |
|---|---|
| **域名规则** | 按 URL 模式追加系统提示词（例如在 `github.com` 上始终用英文回复） |
| **脱敏规则** | 发送前按正则脱敏内容（例如去掉手机号） |
| **回复语言** | 无论页面语言，强制用指定语言回复 |
| **最大文本字符数** | 限制每轮发送多少页面内容 |
| **长附件自动总结** | 超过阈值（默认 100,000 字符）的页面或字幕会分块、并行总结、后台合并——附加立即返回，后续轮次只发压缩版；`[mm:ss]` 标记显式保留，跳转链接继续可用；任何错误都安全失败、静默保留原文 |
| **llms.txt** | 附加页面（📎）时抓取一次 `<origin>/llms.txt`，把站点 LLM 指令烘入页面上下文——放在系统提示词之外，保证提示词前缀跨轮次字节稳定（对 prompt 缓存友好） |
