...
- **长附件自动总结**——当附加的页面或视频字幕超过阈值（默认 40,000 字符）时，browsa 会分块、用当前配置的 provider 并行总结每个块、再在后台合并一次——附加响应立即返回无延迟，后续轮次使用压缩版而不是每次重发全文。视频字幕的时间戳标记（`[mm:ss]`）会显式保留，可点击跳转链接继续可用。任何错误都安全失败，静默保留原文。
