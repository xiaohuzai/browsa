...
(function () {
  ...
  (async () => {
    try {
      ...
      for (const el of shadow.querySelectorAll('[data-i18n]')) { const v = m(el.dataset.i18n); if (v) el.textContent = v; }
      for (const el of shadow.querySelectorAll('[data-i18n-title]')) { const v = m(el.dataset.i18nTitle); if (v) el.title = v; }
      for (const el of shadow.querySelectorAll('[data-i18n-aria]')) { const v = m(el.dataset.i18nAria); if (v) el.setAttribute('aria-label', v); }
