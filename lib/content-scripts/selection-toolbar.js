...
(function () {
  ...
  popShadow.innerHTML = `
    ...
    <div class="pop" id="pop" role="dialog" aria-label="${M('inlineExplainAria', 'Explanation')}" data-i18n-aria="inlineExplainAria">
      ...
      <div class="pop-foot" id="pop-foot">
        ...
        <button class="fbtn" id="pop-side" data-i18n="inlineExplainSide">${M('inlineExplainSide', 'Follow up in panel')}</button>
  ...
  // Debounced + empty-skipped: selectionchange fires on every caret move in
  // contenteditable (i.e. per keystroke in Docs) and on every drag frame —
  // an ungated sendMessage per event cold-starts the service worker each
  // time. Empty text is dropped here because the background handler ignores
  // it anyway (`if (tabId && msg.text)`), so skipping it loses nothing.
  let pendingSelectionCache = null;
  document.addEventListener('selectionchange', () => {
    clearTimeout(pendingSelectionCache);
    pendingSelectionCache = setTimeout(() => {
      pendingSelectionCache = null;
  ...
  // Flush the debounced cache write on mouseup: the selection is final by
  // then, so a user who finishes a selection and immediately (<220ms) clicks
  // 📎 in the panel still reads a fresh cache — with the trailing-only
  // debounce the write landed AFTER the attach's cache read ("No text
  // selected" on a fast select→attach). One extra send per COMPLETED
  // selection at most (empty selections send nothing), still nothing per
  // caret-move/drag-frame — the wake-cost win the debounce exists for.
  document.addEventListener('mouseup', () => {
    if (pendingSelectionCache == null) return;
    clearTimeout(pendingSelectionCache);
    pendingSelectionCache = null;
    const text = (window.getSelection()?.toString() || '').trim();
    if (!text) return;
