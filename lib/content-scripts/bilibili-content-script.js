...
function readBilibiliMediaStreams() {
  try {
    ...
    // size 直接映射（API 有该字段，字节）——模式选择卡的体积预估靠它；duration
    // 不映射：muxed 流没有 bandwidth，bw×dur 推算恒为 0，而 length 的 ms/s 量纲
    // 存疑，映射了反而可能喂给截断校验一个错误值。
    if (Array.isArray(pi.durl) && pi.durl.length > 0) {
      const d = pi.durl[0];
      if (d.url) streams.push({ type: 'muxed', label: 'mp4', url: d.url, hasAudio: true, size: d.size || 0 });
    }
    return streams;
  } catch (_) { return []; }
}
...
async function fetchFreshBilibiliStreams(bvid, cid) {
  ...
  const streams = [];
    ...
    if (Array.isArray(data.durl) && data.durl.length > 0 && data.durl[0].url) {
      streams.push({ type: 'muxed', label: 'mp4', url: data.durl[0].url, hasAudio: true, size: data.durl[0].size || 0 });
    }
  return streams;
}
