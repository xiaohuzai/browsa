...
function readBilibiliMediaStreams() {
  try {
    ...
    // Dash video: video-only streams (no audio track).
    if (Array.isArray(pi.dash?.video)) {
      for (const v of pi.dash.video) {
        ...
        streams.push({ type: 'video', label, url, bandwidth: v.bandwidth || 0, hasAudio: false });
      }
    }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { isBilibiliViewUrl, isBilibiliPlayerUrl, extractBilibiliVideo, installBilibiliInterceptor, md5, wbiSign, activeFetchBilibiliVideo, fetchBilibiliSubtitle, readBilibiliMediaStreams };
}
