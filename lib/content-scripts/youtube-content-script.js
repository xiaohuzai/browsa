...
// Read downloadable audio/video stream URLs from
// window.ytInitialPlayerResponse.streamingData. formats[] are muxed (audio+video
// together, hasAudio true); adaptiveFormats[] are separated into audio-only and
// video-only streams. v1 only returns streams that already carry a usable `url` -
// signatureCipher-only streams (which need the player's JS to decrypt the
// signature) are skipped, since decrypting them in-page is fragile and out of
// scope. Self-contained: only reads window.ytInitialPlayerResponse, no sibling
// calls - safe for chrome.scripting.executeScript({world:'MAIN'}) injection.
function readYouTubeStreams() {
  try {
    const sd = window.ytInitialPlayerResponse?.streamingData;
    if (!sd) return [];
    const streams = [];
    // formats: muxed (audio+video together) - preferred for "download video".
    if (Array.isArray(sd.formats)) {
      for (const f of sd.formats) {
        ...
        streams.push({
          ...
          url: f.url, mimeType: f.mimeType || '', hasAudio: true, hasVideo: true
        });
      }
    }
    // adaptiveFormats: separated audio / video streams.
    if (Array.isArray(sd.adaptiveFormats)) {
      for (const f of sd.adaptiveFormats) {
        if (!f?.url) continue;
        const mt = f.mimeType || '';
        if (mt.startsWith('audio/')) {
          const q = f.audioQuality ? f.audioQuality.replace('AUDIO_QUALITY_', '').toLowerCase() : (f.bitrate ? Math.round(f.bitrate / 1000) + ' kbps' : 'audio');
          streams.push({ type: 'audio', label: q, url: f.url, mimeType: mt, hasAudio: true, hasVideo: false });
        } else if (mt.startsWith('video/')) {
          const label = f.qualityLabel || (f.width && f.height ? `${f.width}x${f.height}` : 'video');
          streams.push({ type: 'video', label, url: f.url, mimeType: mt, hasAudio: false, hasVideo: true });
        }
      }
    }
    return streams;
...
// Fetch FRESH audio stream URLs for ASR. The /youtubei/v1/player response
// (same ANDROID client POST as activeYouTubeFetch Stage 3) embeds the current
// streamingData with a fresh PO token / signature, unlike the SSR globals
// (ytInitialPlayerResponse goes stale after SPA navigation). Mirrors
// fetchFreshBilibiliStreams' return shape: audio-only adaptiveFormats that
// carry a bare `url` (signatureCipher-only streams need the player JS to
// decrypt — skipped, same rule as readYouTubeStreams). Returns an object so
// the caller also gets the video's true length (for truncated-stream detection).
// Self-contained: only fetch / window.ytcfg / URL builtins, no sibling calls —
// safe for chrome.scripting.executeScript({world:'MAIN'}) injection.
async function fetchFreshYouTubeStreams(videoId) {
  try {
    if (!videoId) return { streams: [], videoDurationSec: 0 };
    ...
    const res = await fetch(playerUrl, {
      ...
      credentials: 'include',
      body: JSON.stringify({ context: androidContext, videoId, racyCheckOk: false, contentCheckOk: false })
    });
    if (!res.ok) return { streams: [], videoDurationSec: 0 };
    const data = await res.json();
    const sd = data?.streamingData;
    const lengthSeconds = parseInt(data?.videoDetails?.lengthSeconds || '0', 10);
    if (!sd) return { streams: [], videoDurationSec: lengthSeconds || 0 };
    const streams = [];
    if (Array.isArray(sd.adaptiveFormats)) {
      for (const f of sd.adaptiveFormats) {
        if (!f?.url) continue; // skip signatureCipher-only (needs JS decrypt)
        const mt = f.mimeType || '';
        if (!mt.startsWith('audio/')) continue;
        const codecs = (mt.match(/codecs="([^"]*)"/) || [])[1] || '';
        const q = f.audioQuality ? f.audioQuality.replace('AUDIO_QUALITY_', '').toLowerCase()
        ...
        streams.push({
          ...
          // adaptiveFormats don't carry a duration field — the completeness guard
          // uses the sidepanel's post-transcode WAV-duration check against
          // videoDurationSec instead (same as the bilibili unknown-duration path).
          duration: 0,
    ...
    return { streams, videoDurationSec: lengthSeconds || 0 };
  } catch (_) { return { streams: [], videoDurationSec: 0 }; }
}

// Read audio-only adaptive-format stream URLs from a PLAYER RESPONSE object.
// Unlike the ANDROID fresh-POST path (fetchFreshYouTubeStreams, which returns
// pot-LESS URLs that googlevideo 403s — real test 2026-08-25), the page's own
// WEB player response carries `pot` (PO token) baked into each `url`, because
// the real player obtained it via its own botguard flow. These pot URLs are the
// ones the player itself successfully fetches to play the video. We capture them
// from the intercepted /youtubei/v1/player response (handlePlayerResponse) so
// ASR can try them before falling back to the pot-less ANDROID fetch.
// Self-contained: pure data extraction, no window/closure deps — safe for the
// passive intercept path.
function readPlayerAudioStreams(playerData) {
  try {
    const sd = playerData?.streamingData;
    if (!sd || !Array.isArray(sd.adaptiveFormats)) return [];
    const out = [];
    for (const f of sd.adaptiveFormats) {
      if (!f?.url) continue; // skip signatureCipher-only (needs JS decrypt)
      const mt = f.mimeType || '';
      ...
      out.push({
        type: 'audio',
        label: f.audioQuality ? f.audioQuality.replace('AUDIO_QUALITY_', '').toLowerCase()
          : (f.bitrate ? Math.round(f.bitrate / 1000) + ' kbps' : 'audio'),
        url: f.url,
        mimeType: mt,
        hasAudio: true, hasVideo: false,
        ...
        codecs,
        id: f.itag || 0,
        // pot URLs carry a PO token in the query string — the mark that this
        // stream came from the real player (vs the pot-less ANDROID POST).
        hasPot: /[?&]pot=/.test(f.url),
      });
    }
    return out;
  } catch (_) { return []; }
...
async function activeYouTubeFetch() {
  ...
  try {
    ...
    for (let i = timedtext.length - 1; i >= 0; i--) {
      ...
      const text = await res.text();
      if (!text) continue;
      const lines = parseTimedtextJson(text);
      if (lines) { transcript = lines; break; }
    }
...
// A track's text is served verbatim, and "verbatim" is not plain text:
// creator-uploaded/edited captions carry inline markup (<b>/<i>/<u>/<font
// color=…> — the caption editor's bold/italic/underline buttons write those),
// ASR tracks wrap every word in <s ac="0"> timing tags, and the same markup can
// also arrive entity-escaped (&lt;b&gt;) — which then decodes back into a
// literal tag. Unstripped, it reaches the timeline UI and the model as noise.

// Decodes the entities YouTube emits in timedtext (named + numeric).
function decodeSubtitleEntities(s) {
  // Out-of-range or lone-surrogate code points are dropped rather than thrown
  // on — String.fromCodePoint would raise, and one bad entity inside a caption
  // must not take the whole transcript down (a lone surrogate would also break
...
// Strips inline markup from one subtitle run. Two stages, deliberately
// different in aggressiveness, because the only way this function could lose
// real subtitle text is by mistaking prose for a tag:
//   (1) Real markup straight from the source. XML cannot carry a bare "<" in
//       text (it must be escaped) and json3 writes formatting as literal tags,
//       so anything tag-shaped here is genuinely markup — strip freely.
//   (2) Markup that was entity-escaped in the source. Decoding surfaces it, but
//       decoding is also what could turn entity-encoded prose into something
//       tag-shaped: "x&lt;b and c&gt;y" becomes "x<b and c>y", which no parser
//       can tell apart from a tag. Requiring a MATCHED PAIR separates them —
//       real markup and escaped markup always come in pairs, accidental prose
//       essentially never does. Two rounds so double-escaped markup unwraps too.
function normalizeSubtitleText(raw) {
  let out = String(raw ?? '')
    .replace(/<\/?(?:br|hr)\s*\/?>/gi, ' ')
    .replace(/<\/?[a-zA-Z][^>]*>/g, '');
  for (let round = 0; round < 2; round++) {
    out = decodeSubtitleEntities(out);
    // Repeated so nested markup unwraps innermost-first (<b>x<i>y</i></b>).
    for (let i = 0; i < 5; i++) {
      const next = out.replace(/<([a-zA-Z][a-zA-Z0-9]*)\b[^>]*>([\s\S]*?)<\/\1\s*>/g, '$2');
      if (next === out) break;
      out = next;
    }
  }
...
// TTML timestamps look like "00:00:18.640" (possibly with a trailing "s").
function parseSubtitleClock(value) {
  const parts = String(value).trim().replace(/s$/i, '').split(':').map(Number);
  if (parts.some((p) => !Number.isFinite(p))) return 0;
  return parts.reduce((acc, p) => acc * 60 + p, 0);
}

// Resolves a timedtext element's start time in seconds from whichever
// timestamp attribute its format uses: srv3/legacy `t`/`tStartMs` (ms), srv1/
// srv2 `start` (seconds), TTML `begin` (clock). Returns null when none match.
function parseSubtitleStart(attrs) {
  const ms = /\bt(?:StartMs)?="(\d+)"/.exec(attrs);
  if (ms) return Number(ms[1]) / 1000;
  const start = /\bstart="([\d.]+)"/.exec(attrs);
  if (start) return parseFloat(start[1]);
  const begin = /\bbegin="([^"]+)"/.exec(attrs);
  if (begin) return parseSubtitleClock(begin[1]);
  return null;
}

// Parses a timedtext XML response into "[mm:ss] text" lines. Covers every XML
// shape YouTube serves:
//   <p t="1360" d="1680">[♪♪♪]</p>                — srv3 (content may be plain,
//   <p t="18800" d="7160"><s ac="0">We're</s>…</p> — ASR word-timed runs, or
//                                                    carry inline markup)
//   <p begin="00:00:18.640" end="…">text</p>       — TTML
//   <text start="18.64" dur="3.24">text</text>     — srv1/srv2
// Both element names accept every timestamp attribute (a superset of the
// former regex, which only knew `t=`/`tStartMs=`), so this can never extract
// fewer lines than it used to. The inner capture is [\s\S]*? rather than [^<]*:
// with the single-char negation, a <p> whose text contained a tag matched
// nothing at all, so every word-timed ASR line was silently dropped.
function parseTimedtextXml(text) {
  if (typeof text !== 'string' || !text) return null;
  try {
    const lines = [];
    const push = (seconds, inner) => {
      if (seconds == null) return;
      const txt = normalizeSubtitleText(inner);
      if (txt) lines.push(`${formatSubtitleTimestamp(seconds)} ${txt}`);
    };
    for (const tag of ['p', 'text']) {
      if (lines.length) break;
      const re = new RegExp(`<${tag}\\b([^>]*)>([\\s\\S]*?)<\\/${tag}>`, 'gi');
      let m;
      while ((m = re.exec(text)) !== null) push(parseSubtitleStart(m[1]), m[2]);
    }
...
function parseTimedtextJson(text) {
  ...
  try {
    ...
    for (const ev of (data.events || [])) {
      if (!ev.segs) continue;
      const txt = normalizeSubtitleText(ev.segs.map((s) => s.utf8 || '').join(''));
      if (!txt) continue;
      lines.push(`${formatSubtitleTimestamp((ev.tStartMs || 0) / 1000)} ${txt}`);
    }
    return lines.length > 0 ? lines.join('\n') : null;
  } catch (_) { return null; }
}

// Bounded put for the transcript/audio caches: keeps the latest `max` keys
// (string keys preserve insertion order = oldest first). Without a cap these
// caches accumulate every video seen in the whole SPA session — YouTube
// never full-reloads, so a long session held ~5-10MB of dead transcripts.
const BROWSA_CACHE_MAX = 10;
function browsaCachePut(cacheObj, key, value, max = BROWSA_CACHE_MAX) {
  const keys = Object.keys(cacheObj);
  if (!(key in cacheObj) && keys.length >= max) delete cacheObj[keys[0]];
  cacheObj[key] = value;
}
...
function installYouTubeInterceptor() {
  ...
  async function handlePlayerResponse(data) {
    ...
    if (transcript) browsaCachePut(window.__browsaTranscriptCache, meta.videoId, transcript);
    // Capture pot-bearing audio stream URLs from the REAL player response (the
    // page's WEB client obtained the pot via its own botguard flow). The pot-less
    // ANDROID POST path 403s, so these are the best ASR download candidates.
    // Only cache audio streams that actually carry a pot in the URL — a player
    // response without pot is exactly the dead path we're trying to avoid.
    try {
      const audioStreams = readPlayerAudioStreams(data).filter((s) => s.hasPot);
      if (audioStreams.length) {
        window.__browsaPlayerAudioCache = window.__browsaPlayerAudioCache || {};
        browsaCachePut(window.__browsaPlayerAudioCache, meta.videoId, audioStreams);
        console.log(`browsa: handlePlayerResponse cached ${audioStreams.length} pot audio streams for ${meta.videoId}`);
      }
    } catch (_) {}
    const video = {
        ...
        if (vid) {
          ...
          p.then((r) => r.clone().text())
            .then((text) => {
              ...
              if (lines) browsaCachePut(window.__browsaTranscriptCache, vid, lines);
            })
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { isYouTubePlayerUrl, extractVideoMeta, fetchTranscript, readYouTubeRichMeta, readYouTubeStreams, readPlayerAudioStreams, fetchFreshYouTubeStreams, extractYouTubeChapters, activeYouTubeFetch, installYouTubeInterceptor, isTimedtextUrl, extractVideoIdFromTimedtextUrl, parseTimedtextJson, parseTimedtextXml, normalizeSubtitleText };
}
...
if (typeof window !== 'undefined') {
  ...
  // Expose fresh audio-stream fetch for ASR (mirrors __browsaFetchFreshBilibiliStreams).
  // Read-only reader stays on the page for the passive path; the fresh fetcher
  // is exposed so the background can re-sign a fresh PO-token'd URL on demand
  // (e.g. expired-playurl self-healing / ASR attach) without a page reload.
  window.__browsaFetchFreshYouTubeStreams = fetchFreshYouTubeStreams;
  // Reader for the pot-bearing audio streams captured from the page's REAL player
  // response (see handlePlayerResponse). Prefer these over the pot-less ANDROID
  // POST for ASR download — googlevideo 403s pot-less URLs (real test 2026-08-25).
  window.__browsaGetPlayerAudioStreams = (videoId) => {
    try { return (window.__browsaPlayerAudioCache || {})[videoId] || []; }
    catch (_) { return []; }
  };
