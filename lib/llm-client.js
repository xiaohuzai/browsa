 ...
 * 会被无声拦腰截断（用户只能不断说“继续”）。16384 远高于常见默认；若模型硬上限
 * 更低会被服务端夹住（退回模型自己的硬上限），不会报错。
 */
...
export async function responsesStream({
  ...
  if (!res.ok) {
    ...
    throw ProviderAPIError(`/v1/responses HTTP ${res.status}: ${errText.slice(0, 500)}`);
  }
...
export async function anthropicStream({
  ...
  if (!res.ok) {
    const errText = await safeReadText(res);
    throw ProviderAPIError(`/v1/messages HTTP ${res.status}: ${errText.slice(0, 500)}`);
  }
  if (!res.body) throw ProviderNetworkError('No response body (stream)');
  ...
  while (true) {
    ...
    while ((idx = buffer.indexOf('\n\n')) !== -1) {
      ...
      if (ev === 'content_block_delta') {
        const delta = obj?.delta?.type === 'text_delta' ? String(obj.delta.text || '') : '';
        if (delta) { full += delta; if (onDelta) onDelta(delta); }
...
export async function ping({ baseUrl, apiKey, model, apiStyle }) {
  ...
  const endpoint = style === 'anthropic' ? '/v1/messages' : style === 'responses' ? '/v1/responses' : '/v1/chat/completions';
  let res;
  try {
