 ...
 */
export async function chatStream({ baseUrl, apiKey, model, messages, onDelta, onToolProgress, onApproval, onClarify, signal, extraHeaders, temperature, maxTokens, reasoning, thinking = 'omit' }) {
  if (!baseUrl) throw ProviderConfigError('baseUrl is required');
  return openSseStream({
    ...buildChatRequest({ baseUrl, apiKey, model, messages, extraHeaders, temperature, maxTokens, reasoning }),
    signal,
    thinking,
    onDelta,
...
function buildChatRequest({ baseUrl, apiKey, model, messages, extraHeaders, temperature, maxTokens, reasoning }) {
  const body = { messages, stream: true, stream_options: { include_usage: true } };
  if (model) body.model = model;
  if (temperature != null) body.temperature = temperature;
  if (maxTokens > 0) body.max_tokens = maxTokens;
  // Per-model reasoning fields (lib/reasoning-levels.js) merge at body top
  // level — the dialect map decides the exact shape.
...
export async function responsesStream({
  ...
  if (!baseUrl) throw ProviderConfigError('baseUrl is required');
  return openSseStream({
    ...buildResponsesRequest({ baseUrl, apiKey, model, input, instructions, extraHeaders, temperature, maxTokens, reasoning }),
    signal,
    thinking,
    onDelta,
...
function buildResponsesRequest({ baseUrl, apiKey, model, input, instructions, extraHeaders, temperature, maxTokens, reasoning }) {
...
export async function anthropicStream({
  ...
  if (!baseUrl) throw ProviderConfigError('baseUrl is required');
  return openSseStream({
    ...buildAnthropicRequest({ baseUrl, apiKey, model, system, messages, extraHeaders, temperature, maxTokens, reasoning }),
    signal,
    thinking,
    onDelta,
...
function buildAnthropicRequest({ baseUrl, apiKey, model, system, messages, extraHeaders, temperature, maxTokens, reasoning }) {
