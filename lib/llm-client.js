...
/**
 * 拉取供应商的模型 ID 列表（options 的「拉取模型」按钮 + Ping 成功后自动检测，
 * cherry-studio 同款能力）。OpenAI 兼容（chat/responses）与 Anthropic 都在
 * GET /models 上给 { data: [{id}] }（Anthropic 用 x-api-key + anthropic-version，
 * endpointUrl 的版本感知拼接对三家一致——base 不带版本段自动补 /v1）。
 * 永不抛错：返回 { ok:true, models:['id',…] } | { ok:false, error:'…' }——检测
 * 失败不值得打断配置流程，用户手动填模型的老路原样可用。
 */
export async function listModels({ baseUrl, apiKey, apiStyle, signal }) {
  ...
  // 形状兼容：OpenAI/Anthropic 官方都是 data[].id；部分网关/代理给 models[] 或裸数组
  ...
  const models = [...new Set(raw
    ...
    .slice(0, 200);
  if (!models.length) return { ok: false, error: '响应里没有模型列表（既无 data[].id 也无 models[]）' };
