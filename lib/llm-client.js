...
export async function chatStream({ baseUrl, apiKey, model, messages, onDelta, onToolProgress, onApproval, onClarify, signal, extraHeaders, temperature, maxTokens, thinking = 'omit' }) {
  ...
  if (!res.ok) {
    ...
    const neg = await renegotiateOutputCap(res, text, { url, headers: buildHeaders(apiKey, extraHeaders), body, budgetKey: 'max_tokens', signal, networkErrLabel: 'Network error' });
    res = neg.res;
    if (!res.ok) {
      throw ProviderAPIError(`HTTP ${res.status}: ${neg.text.slice(0, 500)}`);
    }
  }
  if (!res.body) {
  ...
  let usage = null;
  let finishReason = '';
  // Reasoning-model thinking (see parseSseEvent): 'inline' wraps the whole
  ...
  const emitChunk = ({ delta, think, usage: u, finishReason: fr }) => {
    if (u) usage = u;
    if (fr) finishReason = fr;
    if (think && thinking === 'inline') {
      if (!inReasoning) { inReasoning = true; full += '<thinking>\n'; if (onDelta) onDelta('<thinking>\n'); }
      full += think; if (onDelta) onDelta(think);
    }
    if (delta) {
  ...
  while (true) {
    ...
    while ((idx = buffer.indexOf('\n\n')) !== -1) {
      ...
      emitChunk(parseSseEvent(event));
    }
  }

  // Flush any trailing single-newline leftover
  emitChunk(parseSseEvent(buffer));
...
function parseSseEvent(eventText) {
  ...
  // Returns { delta, think, usage, finishReason } — delta is the visible
  // content, think the RAW reasoning text (DeepSeek-style `reasoning_content`,
  // or `reasoning` on some gateways) that the caller wraps in <thinking>.
  let acc = '';
  let thinkAcc = '';
  let usage = null;
  let finishReason = '';
...
export async function runsApiStream({
  ...
  outer: while (true) {
    const { value, done } = await reader.read();
    if (done) {
      // reader.cancel() (from the abort listener) makes the pending read()
      // resolve with done=true — it does NOT reject. Re-throw here or an
      // aborted run resolves normally and the half reply gets persisted to
      // history. Same guard chatStream/responsesStream/anthropicStream carry.
...
export async function responsesStream({
  ...
  if (!res.ok) {
    ...
    const neg = await renegotiateOutputCap(res, errText, { url, headers: buildHeaders(apiKey, extraHeaders), body, budgetKey: 'max_output_tokens', signal, networkErrLabel: '/v1/responses request failed' });
    res = neg.res;
    if (!res.ok) {
      throw ProviderAPIError(`/v1/responses HTTP ${res.status}: ${neg.text.slice(0, 500)}`);
    }
  }
...
export async function anthropicStream({
  ...
  if (!res.ok) {
    const errText = await safeReadText(res);
    const neg = await renegotiateOutputCap(res, errText, { url, headers: buildAnthropicHeaders(apiKey, extraHeaders), body, budgetKey: 'max_tokens', signal, networkErrLabel: '/v1/messages request failed' });
    res = neg.res;
    if (!res.ok) {
      throw ProviderAPIError(`/v1/messages HTTP ${res.status}: ${neg.text.slice(0, 500)}`);
    }
  }
  if (!res.body) throw ProviderNetworkError('No response body (stream)');
