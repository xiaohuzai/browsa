...
//      user's own tab is never navigated away or clicked.
//   2. Interactive expansion — LLM-in-the-loop (page-agent's text-DOM recipe)
//      on a BACKGROUND COPY of the page (same URL, same cookies): number the
//      non-navigating controls, ask the ACTIVE PROVIDER for one JSON action
//      {click:{index}} | {done}, click with a danger-word veto, settle on a
//      DOM sentinel (crawl4ai's content-freshness check), feed the
//      observation back. The user's tab is never touched (watching the page
//      click itself was reported as unsettling); the copy's re-extraction is
//      adopted only when it meaningfully beats the baseline, so a degraded
//      hidden-tab copy can never make the result worse. Works with every
//      provider style browsa speaks — the brain is a plain one-shot
//      completion, no agent/tool protocol.
//
...
export async function maybeDeepExtract({ tabId, ctx, textCap, query, redoMode, sendProgress } = {}) {
  ...
  try {
    ...
    // Interactive expansion runs on a BACKGROUND COPY of the page (same URL,
    // same cookies), never on the user's own tab. The copy is only opened
    // when detectIncompleteness actually saw something interactive
    // (load-more / leftover expanders); URL-only pagination goes straight
    // to the walker below.
    let finalText = ctx.text || '';
    let clicks = 0;
    if ((signals.loadMore || signals.expandersLeft > 0) && ctx.meta?.url) {
      const bg = await chrome.tabs.create({ url: ctx.meta.url, active: false }).catch(() => null);
      if (bg) {
        try {
          if (await waitTabComplete(bg.id, 15_000, controller.signal)) {
            // SPAs mount their controls after the load event.
            await new Promise((r) => setTimeout(r, 800));
            try {
              await chrome.scripting.executeScript({ target: { tabId: bg.id }, func: preExtractCleanup, world: 'MAIN' });
            } catch (_) { /* best-effort */ }
            clicks = await driveInTabInteractions({ tabId: bg.id, ask, timeLeft, signal: controller.signal, progress });
            // Adopt the copy's re-extraction only when it meaningfully beat
            // the baseline — a degraded hidden-tab copy can never make the
            // result worse.
            if (clicks > 0) {
              const redo = await inject(bg.id, extractorFor(redoMode), { htmlCap: textCap, query }).catch(() => null);
...
/** Brain-driven click loop over the BACKGROUND COPY tab. Returns the click count. */
