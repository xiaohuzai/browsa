...
/**
 * Non-streaming chat. Returns { full, usage } where usage may be null.
 */
export async function chat({ baseUrl, apiKey, model, messages, signal, extraHeaders, temperature, maxTokens }) {
...
// Keep backward-compatible alias used by old internal callers
function parseSseDelta(eventText) { return parseSseEvent(eventText).delta; }
    ...
    while ((idx = buffer.indexOf('\n\n')) !== -1) {
      ...
      try {
        ...
        if (type === 'response.output_text.delta') {
          ...
          // (populating `full`), but post-tool text only appears here in
          // response.completed. We must emit the remainder even when full != ''.
          ...
          if (output.length) {
            ...
            if (completedText.length > full.length) {
              ...
              const remainder = completedText.slice(full.length);
              full += remainder;
              onDelta(remainder);
            } else if (completedText && !full) {
              full += completedText;
