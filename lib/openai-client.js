...
export async function chat({ baseUrl, apiKey, model, messages, signal, extraHeaders }) {
  ...
  const body = { model, messages, stream: false, enable_tools: true };
  let res;
  try {
    res = await fetch(url, {
      method: 'POST',
...
export async function chatStream({ baseUrl, apiKey, model, messages, onDelta, onToolProgress, signal, extraHeaders }) {
  ...
  const url = joinUrl(baseUrl, '/v1/chat/completions');
  const body = { model, messages, stream: true, enable_tools: true };
  let res;
  try {
    res = await fetch(url, {
      method: 'POST',
 ...
 */
export async function responsesApiStream({ baseUrl, apiKey, model, input, instructions, conversation, onDelta, onToolProgress, signal }) {
  if (!baseUrl) throw ProviderConfigError('baseUrl is required');

  const url = joinUrl(baseUrl, '/v1/responses');
  const body = { model: model || 'hermes-agent', input, stream: true, store: true };
...
/**
 * Hermes /v1/runs API — long-form session streaming with lifecycle management.
 * Supports detach/reattach via run_id, and graceful cancellation.
 *
 * Returns { runId, result } where result is the full assistant text.
 */
export async function runsApiStream({ baseUrl, apiKey, model, input, instructions, conversation, onDelta, onToolProgress, signal }) {
  if (!baseUrl) throw ProviderConfigError('baseUrl is required');

  // 1. Create the run
  const createBody = { input, stream: false };
  if (model) createBody.model = model;
  if (instructions) createBody.instructions = instructions;
  if (conversation) createBody.conversation = conversation;

  let runId;
  try {
    const createRes = await fetch(joinUrl(baseUrl, '/v1/runs'), {
      method: 'POST',
      headers: buildHeaders(apiKey),
    ...
    if (!createRes.ok) {
      ...
      throw ProviderAPIError(`HTTP ${createRes.status}: ${text.slice(0, 500)}`);
    }
    const run = await createRes.json();
    runId = run.run_id;
    if (!runId) throw ProviderAPIError('No run_id in response');
  } catch (e) {
  ...
  const eventsUrl = joinUrl(baseUrl, `/v1/runs/${runId}/events`);
  let eventsRes;
  try {
  ...
  if (!eventsRes.ok) {
    const text = await safeReadText(eventsRes);
    throw ProviderAPIError(`HTTP ${eventsRes.status}: ${text.slice(0, 500)}`);
  }
  if (!eventsRes.body) throw ProviderNetworkError('No response body (stream)');
    ...
    while ((idx = buffer.indexOf('\n\n')) !== -1) {
      ...
      try {
        ...
        if (type === 'response.output_text.delta') {
          ...
          const text = obj.description || obj.text || obj.tool || JSON.stringify(obj);
          onToolProgress(obj.tool ? `${obj.tool}: ${obj.description || ''}`.trim() : text);
        } else if (type === 'response.completed') {
          if (!full && obj.output) {
  ...
  return { runId, result: full };
}

/**
 * Stop a running Hermes run gracefully.
 */
export async function stopRun({ baseUrl, apiKey, runId }) {
  if (!baseUrl || !runId) return;
  try {
    await fetch(joinUrl(baseUrl, `/v1/runs/${runId}/stop`), {
      method: 'POST',
      headers: buildHeaders(apiKey)
    });
  } catch (_) {}
