 ...
 */
export async function chat({ baseUrl, apiKey, messages, signal, extraHeaders }) {
  if (!baseUrl) throw ProviderConfigError('baseUrl is required');

  const url = joinUrl(baseUrl, '/v1/chat/completions');
  const body = { messages, stream: false, enable_tools: true };
  let res;
  try {
    res = await fetch(url, {
      method: 'POST',
 ...
 */
export async function chatStream({ baseUrl, apiKey, messages, onDelta, onToolProgress, signal, extraHeaders }) {
  if (!baseUrl) throw ProviderConfigError('baseUrl is required');

  const url = joinUrl(baseUrl, '/v1/chat/completions');
  const body = { messages, stream: true, enable_tools: true };
  let res;
  try {
    res = await fetch(url, {
      method: 'POST',
