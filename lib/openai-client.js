...
export async function chat({ baseUrl, apiKey, model, messages, signal }) {
  if (!baseUrl) throw new ProviderConfigError('baseUrl is required');
  if (!model) throw new ProviderConfigError('model is required');
  ...
  try {
    ...
    throw new ProviderNetworkError(`Network error: ${e.message}`);
  }

  if (!res.ok) {
    const text = await safeReadText(res);
    throw new ProviderAPIError(`HTTP ${res.status}: ${text.slice(0, 500)}`);
  }
...
export async function chatStream({ baseUrl, apiKey, model, messages, onDelta, signal }) {
  ...
  if (!res.body) {
    throw new ProviderNetworkError('No response body (stream)');
  }
