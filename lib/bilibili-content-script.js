...
// Read the best audio stream URL from window.__playinfo__ (embedded in
// the page HTML by B站). Prefers dash audio; falls back to durl (older
// format for some videos). Returns null if not available.
// The CDN URL is signed and valid for the current browser session.
// Callers must send Referer: https://www.bilibili.com when downloading.
function readBilibiliAudioUrl() {
  try {
    const pi = window.__playinfo__?.data;
    if (!pi) return null;
    // Dash format: separate audio stream (preferred — audio-only, smaller file)
    const dashAudio = pi.dash?.audio;
    if (Array.isArray(dashAudio) && dashAudio.length > 0) {
      return dashAudio[0].base_url || dashAudio[0].baseUrl || null;
    }
    // durl format: single muxed video+audio stream (older non-dash videos)
    const durl = pi.durl;
...
function installBilibiliInterceptor() {
  ...
  async function handleViewResponse(data) {
    ...
    if (!meta) return;
    // Capture audio URL from __playinfo__ while page context is live.
    const audioUrl = readBilibiliAudioUrl();
...
const WBI_ENC_TAB = [46,47,18,2,53,8,23,32,15,50,10,31,58,3,45,35,27,43,5,49,33,9,42,19,29,28,14,39,12,38,41,13,37,48,7,16,24,55,40,61,26,17,0,1,60,51,30,4,22,25,54,21,56,59,6,63,57,62,11,36,20,34,44,52];
...
async function activeFetchBilibiliVideo() {
  ...
  if (!vd?.bvid) throw new Error('no __INITIAL_STATE__.videoData');

  const video = {
    bvid: vd.bvid,
    title: (vd.title || '').trim(),
    desc: (vd.desc || '').trim(),
    author: (vd.owner?.name || '').trim(),
    tname: (vd.tname || '').trim(),
    duration: vd.duration || 0,
    cid: vd.pages?.[0]?.cid || vd.cid || 0,
    stat: {
      view: vd.stat?.view || 0,
      like: vd.stat?.like || 0,
      coin: vd.stat?.coin || 0,
      favorite: vd.stat?.favorite || 0,
      reply: vd.stat?.reply || 0,
    },
    rawAt: Date.now()
  };

  // Capture audio URL from __playinfo__ — available since page already loaded.
  const audioUrl = readBilibiliAudioUrl();
  if (audioUrl) video.audioUrl = audioUrl;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { isBilibiliViewUrl, isBilibiliPlayerUrl, extractBilibiliVideo, installBilibiliInterceptor, md5, wbiSign };
}
