...
export async function extractActiveTab({ mode = 'reader', maxTextChars, xhsXhrNote = null, siteCache = null } = {}) {
  ...
  // YouTube active fallback: same pattern as Bilibili — when passive
  // interception cache is empty, call activeYouTubeFetch() defined in
  // the content script via executeScript(world:'MAIN').
  ...
  // Last-resort safety net: the upfront document.contentType check above is
  // fast but not guaranteed to catch every PDF-without-".pdf-in-the-URL" case
  // (e.g. if executeScript failed). If extraction still came back empty,
  // double-check via the actual HTTP Content-Type header before giving up.
