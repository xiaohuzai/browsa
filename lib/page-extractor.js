...
import { synthesizeSiteCache, synthesizeYouTubeResult, synthesizeBilibiliResult, synthesizeTwitterResult, synthesizeRedditResult } from './site-synthesizers.js';
import { tryXhsExtraction } from './xhs-extractor.js';
import { tryFeishuExtraction } from './feishu-extractor.js';
...
//   5. Return { markdown (from text), imageCount, ... }
export function extractInPageWorld({ mode, htmlCap }) {
  ...
  // 3c) Lazy-load placeholders: WeChat 公众号 (and many CMS themes) render
  //     below-fold images as a 1x1 data:image/svg+xml spacer with the real
  //     URL parked in data-src until scroll. Turndown emits the spacer src,
  //     so below-fold figures extracted as multi-hundred-char data:-URI
  //     noise lines and never reached the [图N] image pipeline (2026-08-31
  //     user report: a WeChat article produced 7 placeholder lines while
  //     every real in-article figure was missed). Swap in the first
  //     absolute http(s) URL from the common lazy attributes -- only when
  //     the current src is empty or a data:/blob: placeholder, so normal
  ...
  try {
    docClone.querySelectorAll('img').forEach((img) => {
      ...
      for (const attr of ['data-src', 'data-original', 'data-lazy-src', 'data-lazy']) {
        ...
        if (/^https?:\/\//i.test(v)) { img.setAttribute('src', v); break; }
      }
    });
  } catch (_) {
    // no data-src available -- leave placeholders as-is
  }
  ...
  try {
    ...
    function postProcessMarkdown(md) {
      ...
      // URL-encoded / angle-bracketed forms (WeChat's 1x1 SVG spacer is
      // data:image/svg+xml,%3C… with \( \) escapes) slip past the base64
      // rule above -- collapse any STANDALONE line that is just one
      // data:/blob: image. Lines where an image is inline within text are
      // left alone (vanishingly rare, and the alt text there may matter).
      out = out.split(/\r?\n/).map((line) => {
        const m = /^\s*!\[([^\]]*)\]\(\s*<?(?:data|blob):/.exec(line);
        return m ? `![${m[1]}](<image-removed>)` : line;
      }).join('\n');
...
export function extractDomTreeInPageWorld({ htmlCap, query }) {
  const sel = window.getSelection ? String(window.getSelection() || '') : '';
...
export function extractFullInPageWorld({ htmlCap }) {
  const sel = window.getSelection ? String(window.getSelection() || '') : '';
  // textContent includes all text nodes (even display:none); innerText respects CSS.
  // Some sites (小红书) hide content with CSS — textContent catches it. But
  // script/style text is never prose: SPA frameworks inline tens of KB of
  // <script> (Next.js RSC flight payloads) at the top of <body>, and the raw
  // textContent dump turned an attach of labuladong.online into 124K chars
  // that were half serialized flight-data JSON. Clone-and-strip keeps the
  // hidden-content capture while dropping dead markup text; the live DOM is
  // never touched, and any failure falls back to the unstripped dump.
  let raw = '';
  try {
    const clone = document.body ? document.body.cloneNode(true) : null;
    if (clone) {
      clone.querySelectorAll('script, style, noscript, template').forEach((el) => { try { el.remove(); } catch (_) {} });
      raw = (clone.textContent || '').trim();
    }
  } catch (_) { /* fall back to the raw dump below */ }
  if (!raw) raw = (document.body?.textContent || document.body?.innerText || '').trim();
...
export async function extractActiveTab({ mode = 'reader', maxTextChars, xhsXhrNote = null, siteCache = null, query = '', preClean = true, tabId = null } = {}) {
  ...
  // Office documents (docx/pptx/xlsx/odt/rtf/epub/…): fetch the raw bytes
  // in-tab and hand them to the sidepanel's docling.rs-wasm worker for fully
  // local Markdown conversion (see lib/sidepanel/office-extractor.js).
  // Fail-open to a URL placeholder exactly like the PDF path.
  const office = await tryOfficeExtraction(tab, meta);
  ...
  // X/Twitter: the passive interceptor never runs (chrome.runtime is undefined
  // in MAIN world — same lesson as YouTube), so SITE_CACHES.twitter is never
  // populated and tweet pages would otherwise fall through to the generic
  // DOM-tree dump (which for X's div-based layout includes all the nav/trending/
  // sidebar chrome). Active fallback reads the tweet conversation out of
  // window.__INITIAL_STATE__ (or the rendered DOM), like the XHS path.
  const xtw = await tryXActiveFallback(tab, meta);
  if (xtw) return xtw;

  // Reddit: no passive interceptor exists (componentized SPA, no simple XHR
  // to hook), so pages fall through to the generic DOM-tree dump which pulls
  // in the whole chrome (skip links, vote buttons, ads, footer, SML.load
  // noise). Active fallback reads the post + comments out of
  // window.__INITIAL_STATE__ or the shreddit-* web components, like the
  // X/XHS paths.
  const reddit = await tryRedditActiveFallback(tab, meta);
  ...
  // Feishu / Lark document fast path: feishu renders docs through a
  // slate-style editor with virtual scrolling — Readability/Turndown flatten
  // tables and miss lazy-rendered blocks. Extract the block structure to
  // clean Markdown instead. Fail-open -> null falls through to generic.
  const feishu = await tryFeishuExtraction(tab, meta, textCap, result);
...
// ── Office-document attach (docx/pptx/xlsx/odt/rtf/epub/…) ─────────────────
// Mirrors the PDF flow byte-fetch-for-byte: URL-extension gate → MAIN-world
// fetch (page's own cookies) → {mode:'office-pending', officeBase64} handed
// to the sidepanel, whose docling.rs-wasm worker (lib/sidepanel/
// office-extractor.js) converts fully locally; any failure degrades to a
// URL placeholder (mode 'office-url') — a pure enhancement, no new failure
// mode. Legacy .doc/.xls/.ppt are NOT in the gate: the wasm build's
// supported extension list (verified at runtime in the spike) doesn't
// include them, and letting one through would waste a fetch on a guaranteed
// conversion failure. Plain-text-ish web formats (md/txt/html/xml/json) are
// excluded on purpose: those URLs are pages the generic pipeline already
// reads well.

const MAX_OFFICE_BYTES = 30 * 1024 * 1024; // same ceiling as MAX_PDF_BYTES —
// base64 inflates 30 MiB to ~40 MB through chrome.runtime messaging, which
// the pdf flow already proves survives the 64MB message limit.

// Extension gate + filename derivation, module-level PURE (exported for
// tests; safe because neither is ever injected into a page — only
// _fetchOfficeBytesInPageWorld below is executeScript'd).
const OFFICE_EXTS = new Set([
  'docx', 'docm', 'dotx', 'dotm',
  'pptx', 'pptm', 'ppsx', 'potx', 'potm',
  'xlsx', 'xlsm', 'xlsb', 'xltx', 'xltm',
  'odt', 'ott', 'ods', 'ots', 'odp', 'otp',
  'rtf', 'epub', 'eml', 'mht', 'mhtml',
  'csv', 'tsv', 'tex', 'adoc', 'asc', 'vtt'
]);

export function isOfficeUrl(url) {
  const m = /\.([a-z0-9]+)(\?.*)?(#.*)?$/i.test(url || '') ? url.match(/\.([a-z0-9]+)(\?.*)?(#.*)?$/i) : null;
  return !!m && OFFICE_EXTS.has(m[1].toLowerCase());
}

export function officeFilenameFromUrl(url) {
  try {
    const path = new URL(url).pathname;
    const base = path.split('/').pop() || '';
    if (/\.[a-z0-9]+$/i.test(base)) return decodeURIComponent(base);
  } catch (_) { /* relative/invalid URL — fall through */ }
  return 'document';
}

function _fetchOfficeBytesInPageWorld(maxBytes) {
  // Nested helpers, self-contained body — MAIN-world injection constraint
  // (countImages lesson). Same GitHub /blob/→raw rewrite and HTML-rejection
  // refinements as _fetchPdfBytesInPageWorld above.
  function toRawOfficeUrl(url) {
    try {
      const u = new URL(url, location.origin);
      if (u.hostname === 'github.com' || u.hostname === 'www.github.com') {
        const m = u.pathname.match(/^\/([^/]+)\/([^/]+)\/blob\/(.+)$/);
  ...
  function fetchAsBase64(url, credentials) {
    return fetch(url, { credentials })
      .then((resp) => {
        if (!resp.ok) return { error: 'fetch failed: ' + resp.status };
        const ct = (resp.headers && resp.headers.get('content-type')) || '';
        if (/text\/html/i.test(ct)) return { error: 'not an office document (html viewer page): ' + ct };
        return resp.blob().then((blob) => {
          if (blob.size > maxBytes) return { error: 'document too large: ' + blob.size };
          return blob.arrayBuffer().then((buf) => {
            const bytes = new Uint8Array(buf);
            let binary = '';
            const chunkSize = 0x8000;
  ...
  const rawUrl = toRawOfficeUrl(location.href);
  if (rawUrl) return fetchAsBase64(rawUrl, 'omit').catch((e) => ({ error: e.message }));
  return fetchAsBase64(location.href, 'include').catch((e) => ({ error: e.message }));
}

/**
 * Office document behind a direct URL (extension-gated). Fetches raw bytes
 * in-tab and returns {mode:'office-pending', officeBase64, filename}; on any
 * failure returns the same shape of URL-only placeholder the PDF path uses
 * (mode 'office-url'). Returns null when the URL isn't an office document.
 */
async function tryOfficeExtraction(tab, meta) {
  if (!isOfficeUrl(tab.url)) return null;
  const filename = officeFilenameFromUrl(tab.url);
  try {
    const [res] = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: _fetchOfficeBytesInPageWorld,
      args: [MAX_OFFICE_BYTES],
      world: 'MAIN'
    });
    const fetched = res?.result;
    if (fetched?.base64) {
      return {
        meta,
        mode: 'office-pending',
        officeBase64: fetched.base64,
        filename,
        truncated: { rawTextLength: fetched.byteLength, textLength: 0, wasCapped: false }
      };
    }
    if (fetched?.error) {
      console.warn('browsa: office byte fetch failed, falling back to placeholder:', fetched.error);
    }
  } catch (e) {
    console.warn('browsa: office byte fetch threw, falling back to placeholder:', e.message);
  }
  const text = `[Document — agent should fetch and read directly]\nURL: ${tab.url}\nTitle: ${meta.title}`;
  return {
    meta,
    mode: 'office-url',
    text,
...
/**
 * X/Twitter active fallback: inject the content script on-demand then call
 * activeXFetch(). Same on-demand injection pattern as tryYoutubeActiveFallback
 * / tryBilibiliActiveFallback — works on any tab regardless of when it was
 * opened. Returns null (falls through to the generic cascade) when the page
 * isn't a tweet URL or extraction yields nothing.
 */
async function tryXActiveFallback(tab, meta) {
  if (!/twitter\.com\/.+\/status\/|x\.com\/.+\/status\//.test(tab.url || '')) return null;
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: 'MAIN',
      files: ['lib/content-scripts/twitter-content-script.js']
    });
    const [res] = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: 'MAIN',
      func: async () => activeXFetch()
    });
    const tweet = res?.result;
    if (tweet?.text) {
      const synthesized = synthesizeTwitterResult(tweet, meta);
      console.log(`browsa: x active-fetch fallback text=${tweet.text?.length || 0} replies=${Array.isArray(tweet.replies) ? tweet.replies.length : 0}`);
      return synthesized;
    }
  } catch (e) { console.log('browsa: tryXActiveFallback error:', e?.message); }
  return null;
}

/**
 * Reddit active fallback: inject the reddit content script on-demand then call
 * activeRedditFetch(). Same on-demand injection pattern as the other active
 * fallbacks — works on any tab regardless of when it was opened. Returns null
 * (falls through to the generic cascade) when the page isn't a Reddit page or
 * extraction yields nothing.
 */
async function tryRedditActiveFallback(tab, meta) {
  if (!/\.reddit\.com\//.test(tab.url || '')) return null;
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: 'MAIN',
      files: ['lib/content-scripts/reddit-content-script.js']
    });
    const [res] = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: 'MAIN',
      func: async () => activeRedditFetch()
    });
    const data = res?.result;
    if (data?.post?.title) {
      const synthesized = synthesizeRedditResult(data, meta);
      console.log(`browsa: reddit active-fetch fallback title=${data.post.title?.length || 0} comments=${Array.isArray(data.comments) ? data.comments.length : 0}`);
      return synthesized;
    }
  } catch (e) { console.log('browsa: tryRedditActiveFallback error:', e?.message); }
  return null;
}

/**
 * Run before extracting a generic page to dismiss cookie banners, expand
 * folded "read more" sections, and (when the page actually lazy-loads)
 * scroll to trigger that loading. Self-contained
 ...
 * Expansion clicks additionally skip chrome regions (nav/header/footer) —
 * their disclosure widgets are site navigation, not content gates.
 *
 * Invasiveness: the user must never watch the page operate itself (user
 * report). Expansion is bounded and self-bailing; the scroll phase runs
 * ONLY on pages showing lazy-loading/virtualization signals, hidden behind
 * CSS visibility while it runs, and always restores the original position.
 *
 ...
 */
export async function preExtractCleanup() {
  const BUDGET_MS = 3500;
  const start = Date.now();
  const timeLeft = () => BUDGET_MS - (Date.now() - start);
  function sleep(ms) { return new Promise((r) => setTimeout(r, ms)); }

  // Destructive verbs joined the purchase/delete family after a real incident:
  // labuladong.online keeps a 「清除阅读历史」 (clear reading history) button in
  // its sidebar with aria-expanded="false"; Step 2 clicked it and armed a
  // destructive confirmation. `clear`/`reset` need word boundaries — bare
  // substrings would veto a control whose label merely contains them.
  const DANGER_WORDS = /delete|remove|purchase|buy now|checkout|\bpay\b|place order|submit|confirm|\bclear\b|\breset\b|\berase\b|\bwipe\b|\bformat\b|log ?out|sign ?out|删除|清除|清空|重置|注销|退出登录|格式化|购买|下单|支付|提交|确认订单|立即购买/i;
  ...
  const MAX_EXPAND = 8;
  // Known consent-management-platform mounts, market-share ordered — trimmed
  // from crawl4ai's js_snippet/remove_consent_popups.js. These cover vendors
  // the generic [class*=cookie] sweep misses, including shadow-DOM mounts
  // (usercentrics/axeptio) whose host element is not itself a banner.
  // Cross-origin iframe CMPs (Sourcepoint sp_message_iframe) are unreachable
  // from page JS and deliberately not listed.
  const CMP_HOSTS = '#onetrust-banner-sdk, #onetrust-consent-sdk, #CybotCookiebotDialog, '
    + '#didomi-host, #didomi-popup, #quantcast-tcf, #qc-cmp2-container, #qc-cmp2-ui, '
    + '#sp_message_container, #truste-consent-track, #cookie-law-info-bar, #klaro, '
    + '#usercentrics-root, #axeptio_mount, #usercentrics-cmp-ui';

  function safeClick(el) {
    if (!el || typeof el.click !== 'function') return false;
    // An anchor with a real href NAVIGATES the tab. The extraction that
    // follows then dies on a destroyed frame ("Frame with ID 0 was removed")
    // and the whole attach fails. Real report: hfviewer.com's brand link is
    // `<a id="brand" aria-expanded="false" href="/">` -- outside any nav/
    // header, so the chrome exclusion misses it, and its text carries no
    // danger word -- so Step 2 clicked it and left the article for the home
    // page. Same predicate (and same #/javascript: escape hatch for pure JS
    // toggles) interactiveSnapshot already applies to its own controls.
    if (el.tagName === 'A') {
  ...
  // Chrome regions never hold article read-more controls (the same exclusion
  // detectIncompleteness and interactiveSnapshot apply); expanding their
  // disclosure widgets just opens nav mega-menus in front of the user — the
  // reported GitHub incident. Deliberately aligned with
  // extractDomTreeInPageWorld's isChromeNoise — nav/header/footer/aside plus
  // the complementary/search roles: on labuladong.online the ONLY expander
  // candidate outside nav/header was the sidebar's 清除阅读历史 button, sitting
  // in `aside.hidden.lg:block`, and clicking it armed a destructive
  // confirmation. Nested here per the MAIN-world serialization constraint
  // (no sibling-module sharing).
  function inChrome(el) {
    try {
      return !!(el.closest && el.closest('nav, header, footer, aside, [role="navigation"], [role="menubar"], [role="complementary"], [role="search"]'));
    } catch (_) { return false; }
  }

  // A visible dialog means a click armed some site action (confirmations,
  // popups rendered as dialogs) rather than expanded content — and React apps
  // pre-render their dialog text hidden in the DOM, so the body-text sentinel
  // below cannot see it open (same labuladong.online incident: Readability
  // then scored the confirm dialog as the article and the whole cascade fell
  // through to the raw full-text wall). Dismiss whatever popped via an
  // explicitly cancel-labelled control when there is one — NEVER a bare
  // confirm/ok — else programmatically close it. <dialog> answers `.open`;
  // role=dialog divs fall back to checkVisibility (Chrome 105+) then computed
  // style, which jsdom does not really implement (tests exercise the <dialog>
  // path). Nested per the same MAIN-world serialization constraint.
  function dialogVisible(d) {
    if (d.open) return true;
    try { if (typeof d.checkVisibility === 'function' && d.checkVisibility()) return true; } catch (_) {}
    try {
      const cs = window.getComputedStyle ? window.getComputedStyle(d) : null;
  ...
  function dismissOpenDialogs() {
    ...
    try {
      for (const d of document.querySelectorAll('dialog[open], [role="dialog"], [role="alertdialog"]')) {
        if (!dialogVisible(d)) continue;
        let done = false;
        for (const b of d.querySelectorAll('button, [role="button"]')) {
          const t = (b.textContent || '').trim();
          if (/^(cancel|close|dismiss|no|not now|later|取消|关闭|不了|暂不|以后再说)/i.test(t)) {
            try { b.click(); done = true; break; } catch (_) {}
          }
        }
        if (!done) {
          try { if (typeof d.close === 'function') d.close(); else if (d.removeAttribute) d.removeAttribute('open'); } catch (_) {}
          try { d.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true })); } catch (_) {}
        }
        closed++;
      }
    } catch (_) {}
  ...
  // Step 1: cookie/consent banners — container-gated for safety; the
  // selector family spans the generic cookie/consent classes plus known CMP
  // mounts, and shadow hosts get their open shadowRoot searched too.
  try {
    outer:
    for (const c of document.querySelectorAll(COOKIE_CONTAINER_SELECTOR + ', ' + CMP_HOSTS)) {
      if (timeLeft() <= 0) break;
      const roots = [c];
      if (c.shadowRoot) roots.push(c.shadowRoot);
  ...
  // Step 2: expand collapsed content — bounded by MAX_EXPAND, with a
  // zero-delta bail-out (crawl4ai's action-result verification): after two
  // consecutive clicks that add no text, stop — the remaining controls are
  // decoration, not content gates, and clicking them only burns budget.
  try {
    const sig = () => {
      const b = document.body;
      if (!b) return 0;
      // JSDOM test worlds have no innerText; textContent is the fallback.
      const t = typeof b.innerText === 'string' ? b.innerText : (b.textContent || '');
      return t.length;
    };
    let lastSig = sig();
    let stale = 0;
    for (const el of document.querySelectorAll('[aria-expanded="false"], [class*="read-more" i], [class*="show-more" i]')) {
      ...
      if (isExpandable && safeClick(el)) {
        actions.expandedCount++;
        await sleep(Math.min(150, Math.max(30, timeLeft() - 50)));
        // A click that popped a dialog expanded nothing — it armed a site
        // action. Dismiss what it opened and stop the pass entirely; the
        // body-text sentinel cannot catch this itself, because SPA dialog
        // text usually already exists in the DOM, just hidden.
        const popped = dismissOpenDialogs();
        if (popped) {
          // Key appears only when meaningful, same convention as feedItemsRestored.
          actions.dialogsDismissed = (actions.dialogsDismissed || 0) + popped;
          actions.expandedCount--;
          break;
        }
        const now = sig();
  ...
  // Step 3: scroll to bottom to trigger lazy-loaded content — GATED on the
  // page actually showing lazy-loading or virtualization signals. A static
  // article (the majority) must not move at all: the unconditional
  // scroll-jump on every attach was reported as unsettling, and on static
  // pages scrolling accomplishes nothing. When scrolling IS needed, the
  // document is briefly hidden via CSS visibility — paint-only, so layout,
  // scroll geometry, scroll events and IntersectionObserver all keep
  // working — so the user sees a blink instead of the page operating
  // itself; restored in the finally below, with a page-world failsafe timer
  // that un-hides even if this injected script is killed before it runs.
  ...
  let needsLazyScroll = !!feedCandidate;
  if (!needsLazyScroll) {
    try {
      needsLazyScroll = !!document.querySelector(
        'img[loading="lazy"], [data-src], [data-original], [data-lazy-src], .lazyload, [class*="lazyload" i]'
      );
    } catch (_) {}
  }
  ...
  if (needsLazyScroll) {
    ...
    // Paint-suppress with OPACITY, not visibility. visibility is inherited —
    // setting it on <html> flips the computed style of EVERY element in the
    // page, and scroll-reveal pages choreograph against exactly that property:
    // on labuladong.online the suppression race left the article subtree stuck
    // at computed visibility:hidden after the scroll (blanking the user's page
    // AND blinding extractInPageWorld, whose hidden-mark pass then correctly
    // treated the article as hidden and killed reader mode). html's own opacity
    // paints the whole subtree nothing without touching any descendant's
    // computed styles, so the page's own state is never masked.
    const pageRoot = document.documentElement;
    try { if (pageRoot) pageRoot.style.opacity = '0'; } catch (_) {}
    try { setTimeout(() => { try { pageRoot.style.opacity = ''; } catch (_) {} }, 8000); } catch (_) {}
    try {
      const MAX_ROUNDS = 3;
      ...
      try { if (pageRoot) pageRoot.style.opacity = ''; } catch (_) {}
  ...
  // Final sweep: nothing this pass did should leave a dialog open when the
  // real extraction runs — an open modal is high-text-density chrome that
  // Readability will happily score as the article. Covers dialogs opened by
  // anything other than a Step 2 expand click (Step 1's accept, scroll
  // handlers, a pre-existing open dialog). Key only set when non-zero,
  // same convention as feedItemsRestored.
  const swept = dismissOpenDialogs();
  if (swept) actions.dialogsDismissed = (actions.dialogsDismissed || 0) + swept;

  return actions;
}

/**
 * Runs IN THE TAB (MAIN world): after the generic extraction cascade, look
 * for signals that the page still holds content the heuristics could not
 * reach — a URL-pagination control, a same-DOM "load more" button, or
 * collapsed expanders that survived preExtractCleanup's bounded pass. The
 * result rides back on the extraction result as `deepExtractSignals` and
 * gates the escalation in lib/agentic-extract.js. Self-contained (MAIN-world
 * serialization — no sibling calls), sync, and cheap (two selector sweeps).
 */
export function detectIncompleteness() {
  const out = { nextPageHref: null, loadMore: false, expandersLeft: 0 };
  try {
    ...
    // Chrome regions never hold "read the rest of the article" controls;
    // excluding them kills the collapsed-nav-menu false positive.
    const inChrome = (el) => !!el.closest('nav, header, footer, [role="navigation"], [role="menubar"]');

    // 1. Standard rel=next — the one universally safe pagination signal.
    const rel = document.querySelector('link[rel="next"][href], a[rel="next"][href]');
    if (rel) { try { out.nextPageHref = new URL(rel.getAttribute('href'), location.href).href; } catch (_) {} }

    // 2. Anchors scoped to a pagination container whose label says "next".
    //    Bare "›"/"»" symbols are breadcrumb material in the wild, so they
    //    are only trusted inside such a container.
    const NEXT_TEXT = /^(下一页|下一頁|下页|next(\s*page)?|next\s*[›»]|[›»])$/i;
    if (!out.nextPageHref) {
      const containers = document.querySelectorAll(
        '[class*="pagination" i], [class*="pager" i], [id*="pagination" i], [class*="page-nav" i], [class*="pagenav" i]'
      );
      for (const c of containers) {
        for (const a of c.querySelectorAll('a[href]')) {
          if (!visible(a)) continue;
          const label = norm(a.textContent) || norm(a.getAttribute('aria-label')) || norm(a.getAttribute('title'));
          if (NEXT_TEXT.test(label)) {
            try { out.nextPageHref = new URL(a.getAttribute('href'), location.href).href; } catch (_) {}
            break;
          }
        }
        if (out.nextPageHref) break;
      }
    }

    // 3. Global exact-text anchors — CJK labels are unambiguous on their own.
    if (!out.nextPageHref) {
      for (const a of document.querySelectorAll('a[href]')) {
        if (!visible(a) || inChrome(a)) continue;
        if (/^(下一页|下一頁|下页|next page)$/i.test(norm(a.textContent))) {
          try { out.nextPageHref = new URL(a.getAttribute('href'), location.href).href; } catch (_) {}
          break;
        }
      }
    ...
    // A "next" pointing right back here (or a non-http scheme) is no page 2.
    if (out.nextPageHref) {
      try {
        const u = new URL(out.nextPageHref);
        if (u.href === location.href || !/^https?:/.test(u.href)) out.nextPageHref = null;
      } catch (_) { out.nextPageHref = null; }
    }

    // 4. Same-DOM load-more controls (additive by contract — clicking them
    //    appends items without navigating).
    const LOADMORE_TEXT = /(加载更多|載入更多|load more|show more|view more)/i;
    const LOADMORE_CLASS = /load-more|loadmore|show-more|showmore/i;
    for (const el of document.querySelectorAll('button, [role="button"], a, [class*="load-more" i], [class*="show-more" i]')) {
      if (out.loadMore) break;
      if (!visible(el) || inChrome(el)) continue;
      const t = norm(el.textContent);
      if (LOADMORE_CLASS.test(typeof el.className === 'string' ? el.className : '')
        || (t && t.length <= 24 && LOADMORE_TEXT.test(t))) out.loadMore = true;
    }

    // 5. Collapsed expanders still around after preExtractCleanup's pass —
    //    same matcher as its Step 2 so the two counts line up.
    out.expandersLeft = [...document.querySelectorAll('[aria-expanded="false"], [class*="read-more" i], details:not([open]) > summary')]
      .filter((el) => visible(el) && !inChrome(el)).length;
  } catch (_) {}
  return out;
}

/**
 * Runs IN THE TAB (MAIN world): build the numbered control list the
 * deep-extract brain sees, tagging each control with data-browsa-ix so a
 * later clickIndexed() call can re-find it. Only NON-navigating controls are
 * listed — anchors with a real href are excluded outright, so the model can
 * never even point at something that would navigate the user's tab; URL
 * pagination is exclusively the background-tab walker's job. Self-contained.
 */
export function interactiveSnapshot({ maxChars = 5000 } = {}) {
  const out = { text: '', count: 0, items: [] };
  try {
    const norm = (s) => (s || '').replace(/\s+/g, ' ').trim();
    const visible = (el) => {
      const s = getComputedStyle(el);
    ...
    for (const el of document.querySelectorAll(
      'button, [role="button"], [aria-expanded], summary, [onclick], input[type="button"], input[type="submit"], a'
    )) {
      if (items.length >= 150) break;
      if (!visible(el)) continue;
      // Anchors: only href-less (pure JS toggles) are safe to click.
      if (el.tagName === 'A') {
        const href = (el.getAttribute('href') || '').trim();
        if (href && !/^(#|javascript:)/i.test(href)) continue;
      }
      if (el.closest('nav, header, footer, [role="navigation"], [role="menubar"]')) continue;
      const text = norm(el.textContent).slice(0, 80);
      const label = norm(el.getAttribute('aria-label'));
      if (!text && !label) continue;
      el.setAttribute('data-browsa-ix', String(items.length));
      const expanded = el.getAttribute('aria-expanded');
      const head = `[${items.length}]<${el.tagName.toLowerCase()}${expanded != null ? ` aria-expanded=${expanded}` : ''}>`;
      items.push(head + (label && label !== text ? `${label} | ${text}` : text));
    }
    out.count = items.length;
    out.items = items;
    let text = `Page: ${norm(document.title).slice(0, 120)} — ${String(location.href).slice(0, 120)}\nClickable controls:\n${items.join('\n')}`;
...
/**
 * Runs IN THE TAB (MAIN world): re-find the control tagged data-browsa-ix,
 * apply the same danger-word veto preExtractCleanup uses, click it, then
 * sentinel-poll (crawl4ai's content-freshness check): compare a cheap text
 * signature before/after and poll every 100 ms until it changes or the
 * settle budget ends. Returns the next brain observation. Self-contained.
 */
export async function clickIndexed({ index = -1, settleMs = 2000 } = {}) {
  const DANGER = /delete|remove|purchase|buy now|checkout|\bpay\b|place order|submit|confirm|删除|购买|下单|支付|提交|确认订单|立即购买/i;
  const out = { found: false, clicked: false, vetoed: false, changed: false, deltaChars: 0 };
  try {
    const el = document.querySelector(`[data-browsa-ix="${index}"]`);
    if (!el) return out;
    out.found = true;
    const t = (el.textContent || '').trim();
    if (DANGER.test(t)) { out.vetoed = true; return out; }
    // Already-open controls have nothing left to reveal.
    if (el.getAttribute('aria-expanded') === 'true') return out;
    const sig = () => {
      const b = document.body;
      if (!b) return 0;
      const txt = typeof b.innerText === 'string' ? b.innerText : (b.textContent || '');
      return txt.length;
    };
    const before = sig();
    el.click();
...
async function runGenericExtraction(tab, mode, meta, textCap, result, query = '', preClean = true) {
  ...
  try {
    ...
    // A page that navigates (or reloads itself) while we are reading it tears
    // the frame down mid-call and chrome.scripting rejects with "Frame with ID
    // 0 was removed". That raw Chrome string used to reach the user verbatim
    // as "⚠ Failed to read page DOM: Frame with ID 0 was removed." for a tab
    // that was perfectly fine -- once preExtractCleanup's own misclick was
    // fixed (see safeClick), this is what any ordinary redirect or SPA reload
    // inside the ~3.5s cleanup window looked like.
    const inject = (func) => chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func,
    ...
    // The two situations need opposite responses, so they are told apart by
    // URL rather than by error text. Same URL ⇒ the page reloaded itself in
    // place, which is genuinely transient, so retry once. URL moved ⇒ the
    // page the user asked to read is gone; fail with a typed code the side
    // panel can say in the user's own language, rather than silently
    // attaching whatever page replaced it.
    const FRAME_GONE = /frame .*(removed|deleted|gone)|no frame with id/i;
    const run = async (func) => {
      try {
        return await inject(func);
      } catch (e) {
        if (!FRAME_GONE.test(e?.message || '')) throw e;
        const now = await chrome.tabs.get(tab.id).catch(() => null);
        if (now && now.url === tabStill.url) {
          // The new document needs a moment to commit before it can be
          // injected into -- retrying against the dead frame would just throw
          // again (live-verified: 300ms is enough). A different failure on the
          // retry is a real one and propagates as-is.
          await new Promise((r) => setTimeout(r, 300));
          try {
            return await inject(func);
          } catch (e2) {
            if (!FRAME_GONE.test(e2?.message || '')) throw e2;
          }
        }
        const err = new Error(`the page navigated while it was being read (${now?.url || 'tab closed'})`);
        err.code = 'page-navigated';
        throw err;
      }
    };
    ...
    // A typed failure already carries its own reason; re-throw it untouched so
    // the code survives for the side panel to translate, instead of burying it
    // behind the reader-cascade jargon this wrapper exists to explain.
    if (e?.code) throw e;
  ...
  // Content-completeness probe for the deep-extract escalation
  // (lib/agentic-extract.js). One extra cheap executeScript round trip;
  // best-effort — a failed probe just means no escalation. Runs only on
  // this generic path; site fast paths own their extraction entirely.
  try {
    const [probe] = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: detectIncompleteness,
      world: 'MAIN'
    });
    result.deepExtractSignals = probe?.result || null;
