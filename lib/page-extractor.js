...
function extractInPageWorld({ mode, htmlCap }) {
  ...
  try {
    ...
    // Fall back to plain textContent if Turndown blows up
...
export async function ensureReadabilityInjected(tabId) {
  ...
  // Need to inject both. Fetch sources and inject.
  try {
    const [readRes, tdRes] = await Promise.all([
      fetch(chrome.runtime.getURL('lib/Readability.js')),
      fetch(chrome.runtime.getURL('lib/Turndown.js'))
    ]);
    const readabilitySrc = (await readRes.text()).replace(/\nexport\s*\{[^}]*\};?/g, '');
    const turndownSrc = (await tdRes.text()).replace(/\nexport\s*\{[^}]*\};?/g, '');
