...
export async function extractActiveTab({ mode = 'reader', maxTextChars, waitMs = 0 } = {}) {
  ...
  // Auto-detect JS-rendered pages (SPA / React / Vue). If the page body
  // has very little text, it's likely not yet rendered. Poll every 500ms
  // for up to 5 seconds until meaningful content appears.
  if (tab?.id && waitMs === 0) {
    ...
    const charCount = probe[0]?.result || 0;
    if (charCount < 200) {
      // Page looks empty — likely JS rendering still in progress
      console.log(`browsa: page has only ${charCount} chars, waiting for JS render...`);
      for (let i = 0; i < 10; i++) {
        ...
        const len = recheck[0]?.result || 0;
        if (len > 500 || (len > charCount * 3 && len > 200)) {
          console.log(`browsa: content appeared after ${(i+1)*500}ms (${len} chars)`);
          break;
        }
      }
    }
