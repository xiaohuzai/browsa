...
function extractDomTreeInPageWorld({ htmlCap, query }) {
  ...
  // Character-bigram Dice-coefficient score measuring how much `text` overlaps
  // with `query`. Nested here (not a module-level sibling) per the MAIN-world
  // chrome.scripting.executeScript constraint: only this function's own body is
  // serialized on injection, never sibling top-level declarations. Reuses the
  // same bigram technique as lib/dom-similarity.js's stringRatio and the XHS
  // anchor-relocation code, both of which already duplicate it for this reason.
  // CJK-friendly: character bigrams have no word-boundary assumption, unlike a
  // whitespace-split tokenizer. Returns 0–1 (0 = no overlap, 1 = identical sets).
  function scoreTextByQuery(text, q) {
    if (!text || !q) return 0;
    function bigrams(s) {
    ...
    const qBg = bigrams(q);
    const tBg = bigrams(text);
    let dot = 0, qMag = 0, tMag = 0;
    for (const [bg, qc] of qBg) {
      qMag += qc * qc;
      const tc = tBg.get(bg) || 0;
      dot += qc * tc;
    }
    for (const tc of tBg.values()) tMag += tc * tc;
    if (qMag === 0 || tMag === 0) return 0;
    return dot / (Math.sqrt(qMag) * Math.sqrt(tMag));
  }
  ...
  function walk(el, depth) {
    ...
    if (repeatedGroup) {
      ...
      if (query) {
        emitQueue = [...orderedGroupItems].sort((a, b) =>
          scoreTextByQuery(compress(b.child.textContent), query) -
          scoreTextByQuery(compress(a.child.textContent), query)
        );
      }
