...
export async function preExtractCleanup() {
  ...
  // reported GitHub incident. Nested here per the MAIN-world serialization
  // constraint (no sibling-module sharing).
  function inChrome(el) {
    try {
      return !!(el.closest && el.closest('nav, header, footer, [role="navigation"], [role="menubar"]'));
  ...
  if (needsLazyScroll) {
    ...
    try { if (pageRoot) pageRoot.style.visibility = 'hidden'; } catch (_) {}
    try { setTimeout(() => { try { pageRoot.style.visibility = ''; } catch (_) {} }, 8000); } catch (_) {}
    try {
      const MAX_ROUNDS = 3;
      ...
      try { if (pageRoot) pageRoot.style.visibility = ''; } catch (_) {}
