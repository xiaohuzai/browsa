...
export async function extractActiveTab({ mode = 'reader', maxTextChars, waitMs = 0 } = {}) {
  ...
  if (isXhsNote) {
    try {
      // Wait for the SPA's XHR to inject #detail-title / #detail-desc. The
      // detail page is server-side rendered as a skeleton; the actual note
      // text comes in via a follow-up XHR to /api/sns/web/v1/feed. A fixed
      // sleep is unreliable (fast on small notes, slow on 18-image ones),
      // so we poll for the anchors to appear and bail as soon as they do.
      // We give up after a hard cap so we never hang the side panel.
      const xhsWaitMs = 5000;
      const xhsPollMs = 150;
      const [res] = await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: async (waitMs, pollMs) => {
        ...
        args: [xhsWaitMs, xhsPollMs],
        world: 'MAIN'
      });
      const poll = res?.result || {};
      if (poll.waited) console.log(`browsa: xhs anchor poll waited ${poll.waited}ms`);
      ...
      console.warn('browsa: xhs extractor timed out waiting for anchors, falling back to Readability');
    } catch (e) {
