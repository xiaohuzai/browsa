...
const DEFAULTS = {
  ...
  asr: {
    ...
    model: 'doubao-seed-2-1-lite-260915',          // 已开通的音频理解/ASR 模型 ID
...
/**
 * The GET_ALL_KEYS() targeted read WITHOUT getAll()'s DEFAULTS merge /
 * read-time normalization — raw light-key access for callers that want the
 * stored snapshot as-is (getAll() is the semantic view). Same CONTRACT as
 * getAll(): a NEW key joining this surface has to be added to GET_ALL_KEYS
 * (the key set is deliberately untouched by this seam pass).
 */
export async function getLight() {
  return await chrome.storage.local.get(GET_ALL_KEYS());
}
...
export async function appendToHistory(message) {
  await mutateHistory((current) => { current.push(message); return current; });
}
...
// chrome.storage.session boilerplate, factored once (the three agent-session
// families below differ in KEYING and LIFECYCLE POLICY, not in mechanics).
...
export async function getOrCreateHermesSessionId(providerName) {
  ...
  const existing = await sessionGet(key);
...
export async function getOpencodeSessionId(providerName) {
  const id = await sessionGet(`opencodeSessionId_${providerName}`);
  return typeof id === 'string' ? id : null;
}

export async function setOpencodeSessionId(providerName, sessionId) {
  await sessionSet(`opencodeSessionId_${providerName}`, sessionId);
}
...
// lifecycle as the opencode session id: survives SW restarts mid-conversation,
...
export async function getBridgeSessionId(providerName, endpointUrl) {
  const id = await sessionGet(bridgeSessionKey(providerName, endpointUrl));
  return typeof id === 'string' ? id : null;
}

export async function setBridgeSessionId(providerName, sessionId, endpointUrl) {
  await sessionSet(bridgeSessionKey(providerName, endpointUrl), sessionId);
}

export async function clearBridgeSessionId(providerName) {
  // New conversation: drop the legacy single key AND every per-endpoint key
  // for this provider (the caller doesn't know which endpoints exist).
  const base = `bridgeSessionId_${providerName}`;
  const all = await chrome.storage.session.get(null).catch(() => ({}));
  const stale = Object.keys(all).filter((k) => k === base || k.startsWith(`${base}__`));
  if (stale.length) await sessionRemove(stale);
}
...
// like the Hermes session id; cleared alongside history so a new conversation
 ...
 * conversations mid-turn, the live history ANOTHER session's, so the reply
...
export async function appendToSessionHistory(id, entry) {
  ...
  if (idx === -1) return null;
  const history = [...(savedSessions[idx].history || []), entry];
  savedSessions[idx] = { ...savedSessions[idx], history };
  await chrome.storage.local.set({ savedSessions });
  return savedSessions[idx];
}
