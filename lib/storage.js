...
const DEFAULTS = {
  providers: {
    ...
    // Fixed, not user-deletable; default baseUrl matches `opencode serve
    // --port 4096` (bare `opencode serve` picks a RANDOM port, so the user
    // must pin one for browsa to reach it). apiKey is optional (bearer).
    opencode: { type: 'agent', alias: 'OpenCode Agent', baseUrl: 'http://127.0.0.1:4096', apiKey: '', model: '', stream: true, isHermes: false, isOpencode: true, apiStyle: 'chat', temperature: null, maxTokens: 0 },
