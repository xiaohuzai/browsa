...
const DEFAULTS = {
  providers: {
    ...
    // OpenSquilla is the second built-in agent provider — a local desktop
    // agent whose gateway speaks WebSocket RPC (lib/squilla-client.js).
    // Fixed, not user-deletable; baseUrl is the gateway WS URL
    // (ws://127.0.0.1:18791/ws). The gateway's origin guard requires this
    // extension's origin in its cors.allowed_origins — see README.
    ...
    // Codex is the third built-in agent provider — OpenAI Codex (desktop app
    // or CLI) reached over Chrome Native Messaging through the local
    // `codex app-server --stdio` engine (lib/codex-client.js). The NM host
    // (com.agentbridge.codex) is installed once per machine by the standalone
    // agent-bridge installer — browsa never ships or generates installers, it
    // links to agent-bridge from the options card. No baseUrl/apiKey: the
    // connection is local and authenticated by the user's existing Codex
    // login; engine binary discovery is agent-bridge's job. Fixed, not
    // user-deletable.
    codex: { type: 'agent', alias: 'Codex', baseUrl: '', apiKey: '', model: '', stream: true, isHermes: false, isSquilla: false, isCodex: true, apiStyle: 'chat', temperature: null, maxTokens: 0 },
    // WorkBuddy is the fourth built-in agent provider — Tencent's CodeBuddy
    // CLI in its official headless mode (`--input-format stream-json
    // --output-format stream-json -p -y`), reached over Chrome Native
    // Messaging through the same agent-bridge host as Codex
    // (com.agentbridge.codebuddy). No baseUrl/apiKey: the connection is local
    // and authenticated by the user's CodeBuddy login. Fixed, not
    // user-deletable. NOTE: headless mode cannot forward approval prompts
    // (upstream limitation) — the engine runs with -y; the options card says
    // so.
    workbuddy: { type: 'agent', alias: 'WorkBuddy', baseUrl: '', apiKey: '', model: '', stream: true, isHermes: false, isSquilla: false, isCodex: false, isWorkbuddy: true, apiStyle: 'chat', temperature: null, maxTokens: 0 },
...
// browsa no longer fabricates one itself, because `sessions.send` (the send
// interface that honors the cli caller source) requires an existing session.
// Stored in chrome.storage.session like the Hermes session id; cleared
// alongside history so a new conversation starts a fresh gateway session.
export async function getSquillaSessionKey(providerName) {
  const key = `squillaSessionKey_${providerName}`;
  const res = await chrome.storage.session.get(key).catch(() => ({}));
  return typeof res[key] === 'string' ? res[key] : null;
}

export async function setSquillaSessionKey(providerName, sessionKey) {
  const key = `squillaSessionKey_${providerName}`;
  await chrome.storage.session.set({ [key]: sessionKey }).catch(() => {});
}
...
// Codex keeps per-thread transcripts on disk under ~/.codex and resumable via
// thread/resume — same shape as the OpenSquilla session key: the engine holds
// the history, browsa holds just the pointer. Stored in chrome.storage.session
// (cleared alongside history) so a new browsa conversation opens a new Codex
// thread. Unlike the OpenSquilla key, the id is chosen by browsa's
// thread/start call and echoed back via onThreadId.
export async function getCodexThreadId(providerName) {
  const key = `codexThreadId_${providerName}`;
  const res = await chrome.storage.session.get(key).catch(() => ({}));
  return typeof res[key] === 'string' ? res[key] : null;
}

export async function setCodexThreadId(providerName, threadId) {
  const key = `codexThreadId_${providerName}`;
  await chrome.storage.session.set({ [key]: threadId }).catch(() => {});
}

export async function clearCodexThreadId(providerName) {
  const key = `codexThreadId_${providerName}`;
  await chrome.storage.session.remove(key).catch(() => {});
}


// ─── WorkBuddy session identity ──────────────────────────────────────────────
// CodeBuddy headless keeps per-session transcripts; `--resume <id>` (injected
// through the bridge control frame) continues one. Same model as the Codex
// thread id: the engine holds the history, browsa holds the pointer.
export async function getWorkbuddySessionId(providerName) {
  const key = `workbuddySessionId_${providerName}`;
  const res = await chrome.storage.session.get(key).catch(() => ({}));
  return typeof res[key] === 'string' ? res[key] : null;
}

export async function setWorkbuddySessionId(providerName, sessionId) {
  const key = `workbuddySessionId_${providerName}`;
  await chrome.storage.session.set({ [key]: sessionId }).catch(() => {});
}

export async function clearWorkbuddySessionId(providerName) {
  const key = `workbuddySessionId_${providerName}`;
  await chrome.storage.session.remove(key).catch(() => {});
}
