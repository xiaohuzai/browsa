...
const DEFAULTS = {
  providers: {
    ...
    // `codex app-server --stdio` engine (lib/codex-client.js +
    // lib/nm-bridge.sh). No baseUrl/apiKey: the connection is local and
    // authenticated by the user's existing Codex login. codexBin is an
    // optional explicit binary path baked into the generated bridge install
    // command when auto-discovery (PATH → desktop-app managed copy) isn't
    // enough. Fixed, not user-deletable.
    ...
    // Messaging through the same generic bridge as Codex
    // (lib/codebuddy-client.js + lib/nm-bridge.sh). No baseUrl/apiKey: the
    // connection is local and authenticated by the user's CodeBuddy login.
    // codebuddyBin is an optional explicit binary path baked into the
    // generated install command. Fixed, not user-deletable. NOTE: headless
    // mode cannot forward approval prompts (upstream limitation) — the engine
    // runs with -y; the options card says so.
    workbuddy: { type: 'agent', alias: 'WorkBuddy', baseUrl: '', apiKey: '', model: '', stream: true, isHermes: false, isSquilla: false, isCodex: false, isWorkbuddy: true, apiStyle: 'chat', temperature: null, maxTokens: 0, codebuddyBin: '' },
