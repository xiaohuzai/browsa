...
export async function getBridgeSessionId(providerName) {
  const key = `bridgeSessionId_${providerName}`;
  const res = await chrome.storage.session.get(key).catch(() => ({}));
  return typeof res[key] === 'string' ? res[key] : null;
}

export async function setBridgeSessionId(providerName, sessionId) {
  const key = `bridgeSessionId_${providerName}`;
  await chrome.storage.session.set({ [key]: sessionId }).catch(() => {});
}
