...
export async function getAll() {
  ...
  // bridge 卡名升级（2026-09-09）：旧默认名 'Agent Bridge' 读时归一为带
  // Codex / Claude Code 卖点的新标签——agent 卡没有别名编辑入口，旧名只可能
  // 是我们自己的默认值，不会覆盖用户自定的名字。
  if (mergedProviders.bridge?.alias === 'Agent Bridge') mergedProviders.bridge.alias = BRIDGE_CARD_LABEL;
...
export async function getBridgeSessionId(providerName, endpointUrl) {
  const key = bridgeSessionKey(providerName, endpointUrl);
  const res = await chrome.storage.session.get(key).catch(() => ({}));
  return typeof res[key] === 'string' ? res[key] : null;
}
