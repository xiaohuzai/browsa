...
const DEFAULTS = {
  providers: {
    ...
    // bridge is the third built-in agent provider — the user-run local
...
export async function getAll() {
  ...
  // Never surface the heavy payload keys through the config object: `history`
  // (≤300K chars), `savedSessions` (≤50×300K), the attach snapshot map, and the
  // Ark file cache. getAll() feeds GET_CONFIG and every chat/attach turn, and no
  // config consumer reads any of these — spreading them here made each call
  // serialize megabytes nothing looked at.
  const HEAVY_KEYS = ['history', 'savedSessions', 'browsaAttachSnapshots', 'browsaArkFileCache'];
  const light = { ...stored };
