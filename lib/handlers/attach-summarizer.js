...
export async function summarizeLongText({ text, mode, ctx, provider, chunkSizeChars = DEFAULT_CHUNK_CHARS }) {
  const promptArg = ctx || mode;
  const chunks = splitIntoMarkdownChunks(text, chunkSizeChars);
  if (chunks.length === 0) return text;
  if (chunks.length === 1) return await callSummarizer(provider, chunkSummaryPrompt(promptArg), text);
...
export async function maybeSummarizeAttachment({ attachId, ctx, provider, chunkSizeChars }) {
  ...
  try {
    const summarized = await summarizeLongText({ text: ctx.text, mode: ctx.mode, ctx, provider, chunkSizeChars });
