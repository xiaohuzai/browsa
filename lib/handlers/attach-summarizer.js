...
import { buildPageContextText } from '../page-extractor.js';
...
export async function summarizeLongText({ text, mode, provider, chunkSizeChars = DEFAULT_CHUNK_CHARS }) {
  const chunks = splitIntoChunks(text, chunkSizeChars);
...
export async function maybeSummarizeAttachment({ attachId, ctx, provider, chunkSizeChars }) {
  ...
  try {
    ...
    history[idx] = { ...history[idx], content: buildPageContextText(updatedCtx), summarized: true };
