...
import { resolveChatModel } from './provider-resolver.js';

// Deliberately 100K: this is the fidelity/floor knob, not the overflow guard —
// a too-eager default would silently replace big-window users' verbatim
// attachments with digests. Overflowing anyway is handled at SEND time by
// chat-handler's context-overflow self-rescue (stub + one retry), which knows
// the request actually failed; see isContextOverflowError there.
...
// Paper attachments (meta.paper) get research-shaped prompts: chunk summaries
// keep each section's heading + page marker so the merged digest preserves
// document structure, and the merged output keeps [Page N] anchors (the
// wasm/plain extractors stamp them) so later turns can cite page positions.
function isPaperMode(ctx) {
  return !!ctx?.meta?.paper;
}

function chunkSummaryPrompt(ctxOrMode) {
  const mode = typeof ctxOrMode === 'string' ? ctxOrMode : ctxOrMode?.mode;
  if (isVideoMode(mode)) {
    return 'You are compressing one segment of a long timestamped video transcript (lines formatted as "[mm:ss] text") so it can be used to write video notes later. Condense repetitive, filler, and low-information content, merging closely related consecutive lines into a single condensed point. CRITICAL: every point you keep MUST retain its original [mm:ss] timestamp marker exactly as written (use the earliest timestamp among lines merged into one point) — never invent, renumber, or drop timestamp brackets. Output condensed lines in the same "[mm:ss] point" format only — no preamble, no explanation.';
  }
  if (isPaperMode(ctxOrMode)) {
    return 'You are compressing one segment of a research paper so the segments can later be merged into a structured digest. Preserve: the section headings and their hierarchy, the research problem, the proposed method/design, experimental setup and concrete numbers (datasets, metrics, gains), results, and stated limitations. Drop: repetition, transition prose, and citation lists. CRITICAL: keep every "[Page N]" marker on the first line of the content it introduces, exactly as written — never renumber or drop page markers. Output only the condensed text — no preamble, no explanation.';
  }
  return 'You are compressing one segment of a long document so it can be used as context for later questions. Condense it into a shorter version that preserves all key facts, names, numbers, dates, and direct quotes — drop repetition and filler. Output only the condensed text — no preamble, no explanation.';
}

function mergePrompt(ctxOrMode) {
  const mode = typeof ctxOrMode === 'string' ? ctxOrMode : ctxOrMode?.mode;
  if (isVideoMode(mode)) {
    return 'You are given several condensed segments of the same timestamped video transcript, in chronological order. Merge them into one continuous condensed transcript. Keep every [mm:ss] marker exactly as given — never renumber or invent timestamps. Remove only exact duplicate points that appear at segment boundaries. Output only the merged transcript — no preamble, no explanation.';
  }
  if (isPaperMode(ctxOrMode)) {
    return 'You are given several condensed segments of the same research paper, in original order. Merge them into one coherent structured digest organized by the paper\'s own section headings, each heading followed by its key content (problem, method, experiments with concrete numbers, results, limitations). Keep every "[Page N]" marker exactly as given. Remove only duplicated content at segment boundaries. Output only the merged digest — no preamble, no explanation.';
  }
  return 'You are given several condensed segments of the same document, in original order. Merge them into one coherent condensed document, removing duplicate points that appear at segment boundaries. Output only the merged text — no preamble, no explanation.';
}

async function callSummarizer(provider, systemPrompt, userText, all) {
  const result = await chatStream({
    baseUrl: provider.baseUrl,
    apiKey: provider.apiKey,
    // Honor the sidebar's selected model (all.activeModel) — a multi-model
    // provider previously condensed against provider.model (the card's first
    // slot), which could be an unconfigured/weaker model.
    model: resolveChatModel(provider, all) || provider.model,
    messages: [
...
// final merge call. Accepts either a bare mode string ('youtube') or the
// full pageContext (paper prompt selection keys off meta.paper). Throws on
// any failure so the caller can leave the original content untouched rather
// than storing a half-done result.
export async function summarizeLongText({ text, mode, ctx, provider, all, chunkSizeChars = DEFAULT_CHUNK_CHARS }) {
  const promptArg = ctx || mode;
  const chunks = splitIntoMarkdownChunks(text, chunkSizeChars);
  if (chunks.length === 0) return text;
  if (chunks.length === 1) return await callSummarizer(provider, chunkSummaryPrompt(promptArg), text, all);

  const summaries = await Promise.all(
    chunks.map((chunk, i) =>
      callSummarizer(provider, chunkSummaryPrompt(promptArg), `[Segment ${i + 1}/${chunks.length}]\n${chunk}`, all)
    )
  );
  return await callSummarizer(
    provider,
    mergePrompt(promptArg),
...
export async function maybeSummarizeAttachment({ attachId, ctx, provider, all, chunkSizeChars }) {
  if (!attachId || !ctx?.text) return;
  // Agent providers (opencode, agent-bridge) receive the full attachment
  // inline: the server-side agent manages its own context window, and the
  // summarizer's chatStream cannot speak an agent-provider protocol anyway —
  // condensing here would only degrade what the agent sees (same rule as
  // OpenSquilla).
  if (provider?.isOpencode || provider?.isBridge || provider?.isSquilla) return;
  if (!provider?.baseUrl?.trim()) {
    console.warn('browsa: skipping attachment auto-summarize — no active provider configured');
    return;
  }
  try {
    const summarized = await summarizeLongText({ text: ctx.text, mode: ctx.mode, ctx, provider, all, chunkSizeChars });
    ...
    // ATTACH_PDF_CONFIRM / ATTACH_ASR_CONFIRM) through the summarize rewrite -
    // otherwise a figure-bearing attachment that also exceeds the summarize
    // threshold would silently lose its figures when the text body is
    // condensed to a plain string. 交错条目用 interleaveImageParts 重建：压缩后
    // 的文本若仍含 [图N] 锚点，图片回到锚点位置；锚点被摘要丢掉则退化为
    // [text, ...images]（与 PDF 无位置形状一致）。
    // 整段重写在 storage 的历史锁内进行：本次 LLM 调用期间用户新发的消息
    // 不能被我们的写回覆盖（读-改-写必须原子）。
    await storage.mutateHistory((history) => {
      const idx = history.findIndex((m) => m.attachId === attachId);
      if (idx === -1) return null; // entry was removed/undone/truncated in the meantime — nothing to do
      ...
      const newContent = prevImages.length
        ? interleaveImageParts(rebuiltText, prevImages)
        : rebuiltText;
