...
export async function maybeSummarizeAttachment({ attachId, ctx, provider, chunkSizeChars }) {
  ...
  // OpenSquilla receives the full attachment inline (its gateway treats a
  // cli-caller's long text as first-class turn content and manages context
  // server-side), so condensing here would only degrade what the agent sees —
  // and the summarizer's chatStream cannot even reach a ws:// baseUrl.
  if (provider?.isSquilla) return;
  try {
