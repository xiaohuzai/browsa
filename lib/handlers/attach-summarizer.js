...
export async function maybeSummarizeAttachment({ attachId, ctx, provider, chunkSizeChars }) {
  ...
  // Agent providers (opencode) receive the full attachment inline: the
  // server-side agent manages its own context window, and the summarizer's
  // chatStream cannot speak an agent-provider protocol anyway — condensing
  // here would only degrade what the agent sees (same rule as OpenSquilla).
  if (provider?.isOpencode) return;
