...
import { opencodeStream, createOpencodeSession, buildOpencodeTurn } from '../opencode-client.js';
...
export async function handleChat(msg, capabilityHints, choiceRequestHint) {
  ...
  if (isHermes) {
    ...
    opencodeTurn = buildOpencodeTurn(msg, sendHistory);
  } else if (isBridge) {
    ...
    // retains its own transcript across the conversation.
    bridgeSessionId = await storage.getBridgeSessionId(all.activeProvider);
    bridgeTurn = buildOpencodeTurn(msg, sendHistory);
  ...
  const doStream = async (opts = {}) => {
    ...
    if (isBridge) {
      return await bridgeStream({
        ...
        onSessionId: (sid) => {
          bridgeSessionId = sid;
          storage.setBridgeSessionId(all.activeProvider, sid);
        },
  ...
  try {
    ...
    if (replyTruncated && fullReply) {
      ...
      if (isHermes) {
        ...
        // instruction lands right after its own partial reply.
