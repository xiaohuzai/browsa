...
import { buildAgentTurn, withTurnImages } from '../agent-turn.js';
import { providerEntryLabel } from '../provider-display.js';
import { buildMessages, ageStaleAttachments } from '../message-builder.js';
import { resolveProvider, resolveInferenceParams, resolveChatModel, resolveBridgeEndpoint, resolveBridgeApiKey } from './provider-resolver.js';
...
export async function handleChat(msg, capabilityHints, choiceRequestHint) {
  ...
  const replyModelOrEndpoint = isBridge
    ? (bridgeEndpoint || provider.baseUrl)
    : (isHermes || isOpencode) ? '' : (resolveChatModel(provider, all) || '');
