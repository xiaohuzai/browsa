...
export async function handleChat(msg, capabilityHints, choiceRequestHint) {
  ...
  // opencode agent provider (`opencode serve` headless HTTP server) —
  // session-scoped like Hermes: browsa sends only the user's turn, the
  // server keeps its own transcript. See lib/opencode-client.js.
  const isOpencode = !!(provider.isOpencode);
  // agent-bridge provider (user-run local daemon adapting CLI agents — codex
  // first — to the bridge's wire protocol v1; standalone project
  // github.com/xiaohuzai/agent-bridge). Same one-turn-per-send model as
  // opencode: the bridge's agent keeps its own transcript, browsa sends only
  // the user's turn (+ trailing page-context run). See lib/bridge-client.js.
  const isBridge = !!(provider.isBridge);
  ...
  let opencodeSessionId = null; // opencodeStream: server-assigned `ses_…` id
  let opencodeTurn = '';       // opencodeStream: single prompt string
  let opencodeImages = null;   // opencodeStream: image attachments (leg 1 only)
  let bridgeSessionId = null;  // bridgeStream: adapter-assigned agent thread id
  let bridgeTurn = '';         // bridgeStream: single prompt string
  let bridgeEndpoint = '';     // bridgeStream: selected agent endpoint (multi-bridge)
  let bridgeImages = null;     // bridgeStream: image URLs riding this turn (leg 1 only)
  ...
  if (isHermes) {
    ...
    ({ input: runsInput, conversationHistory: runsConvHistory } = buildHermesTurn(msg, sendHistory));
  } else if (isOpencode) {
    ...
    opencodeSessionId = await storage.getOpencodeSessionId(all.activeProvider);
    const opencodeNewSession = !opencodeSessionId;
    if (!opencodeSessionId) {
      ...
      await storage.setOpencodeSessionId(all.activeProvider, opencodeSessionId);
    }
    ...
    ({ text: opencodeTurn, images: opencodeImages } = withTurnImages(agentTurn.text, agentTurn.images));
    ...
    opencodeTurn = withAgentRenderHints(opencodeTurn, opencodeNewSession);
  } else if (isBridge) {
    // Session id comes from the bridge's 'start' event on the FIRST turn
    // (onSessionId below persists it); later turns reuse it so the agent
    // retains its own transcript across the conversation. Keyed PER ENDPOINT:
    // one bridge card fronts one agent per address (serve mode = one bridge
    // per port), so codex and claude threads must never share an id.
    bridgeEndpoint = resolveBridgeEndpoint(provider, all);
    bridgeSessionId = await storage.getBridgeSessionId(all.activeProvider, bridgeEndpoint);
    const turn = buildAgentTurn(msg, sendHistory, { backfill: !!msg.backfill });
    ({ text: bridgeTurn, images: bridgeImages } = withTurnImages(turn.text, turn.images));
    bridgeTurn = withAgentRenderHints(bridgeTurn, !bridgeSessionId);
  } else if (apiStyle === 'responses') {
    responsesInput = buildResponsesInput(msg, sendHistory);
  } else if (apiStyle === 'anthropic') {
    anthropicMessages = buildAnthropicMessages(msg, sendHistory);
  ...
  const replyModelOrEndpoint = resolveReplyModelOrEndpoint(provider, all, bridgeEndpoint);
  ...
  initStreamState(tabId, { providerLabel, providerKey });
  ...
  const doStream = async (opts = {}) => {
    ...
    const onApprovalBridge = (data) => {
      pendingApprovals.set(tabId, {
        kind: 'bridge',
        baseUrl: bridgeEndpoint || provider.baseUrl,
        // 审批回复必须发回同一座桥、带同一把 key —— 按端点解析，不是卡级。
        apiKey: resolveBridgeApiKey(provider, bridgeEndpoint || provider.baseUrl),
        requestId: data.requestId || '',
      });
      pushChunk(tabId, { type: 'APPROVAL', data });
    };
    ...
    if (isBridge) {
      return await bridgeStream({
        ...
        onSessionId: (sid) => {
          bridgeSessionId = sid;
          storage.setBridgeSessionId(all.activeProvider, sid, bridgeEndpoint);
        },
  ...
  try {
    for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
      try {
        ...
        if (!overflowRescued && !isOpencode && !isBridge && isContextOverflowError(e?.message)) {
          overflowRescued = true;
          const { history: stubbedSendHistory, changed } = stubOversizedAttachments(sendHistory);
          if (changed.length) {
            if (isHermes) {
              runsConvHistory = buildRunsConversationHistory(stubbedSendHistory);
            } else if (apiStyle === 'responses') {
              responsesInput = buildResponsesInput(msg, stubbedSendHistory);
            } else if (apiStyle === 'anthropic') {
              anthropicMessages = buildAnthropicMessages(msg, stubbedSendHistory);
            } else {
              messages = buildMessages({
                history: stubbedSendHistory,
                userText: msg.userText,
                pageContext: null,
                withImage: false,
                userImages: msg.images,
    ...
    if (replyTruncated && fullReply) {
      ...
      if (isHermes) {
        ...
        // The server session already holds v1 — a bare "continue" prompt
        // lands in the agent's own context right after its partial reply.
        // Images rode leg 1; resending would duplicate them in the transcript.
        opencodeTurn = contInstruction;
        opencodeImages = null;
      } else if (isBridge) {
        // Same trick: the bridge's agent thread holds v1; a bare continue
        // instruction lands right after its own partial reply. Images rode
        // leg 1 — resending them here would duplicate them in the thread.
        bridgeTurn = contInstruction;
        bridgeImages = null;
      } else {
        // Rebuild the SAME request shape the original turn used — only the
        // chat-style `messages` was rebuilt here before, so responses/anthropic
        // continuations silently resent the ORIGINAL request (regenerating the
        // same reply instead of continuing it).
        const contHistory = [
          ...sendHistory,
        ...
        if (apiStyle === 'responses') {
          responsesInput = buildResponsesInput({ userText: contInstruction }, contHistory);
        } else if (apiStyle === 'anthropic') {
          anthropicMessages = buildAnthropicMessages({ userText: contInstruction }, contHistory);
        } else {
          messages = buildMessages({
            history: contHistory,
    ...
    if (videoSrc && !_TS_PRESENT_RE.test(fullReply) && _NOTES_REQUEST_RE.test(msg.userText || '') && fullReply.length > 50) {
      ...
      if (isHermes) {
        ...
        runsConvHistory = buildRunsConversationHistory(rewriteHistory);
      } else if (isOpencode) {
        // Same trick as continuation: the server session has v1 in its own
        // transcript, so the rewrite ask rides alone.
        opencodeTurn = rewriteInstruction;
        opencodeImages = null;
      } else if (isBridge) {
        bridgeTurn = rewriteInstruction;
        bridgeImages = null;
      } else if (apiStyle === 'responses') {
        responsesInput = buildResponsesInput({ userText: rewriteInstruction }, rewriteHistory);
      } else if (apiStyle === 'anthropic') {
        anthropicMessages = buildAnthropicMessages({ userText: rewriteInstruction }, rewriteHistory);
      } else {
        messages = buildMessages({
  ...
  await storage.appendToHistory({ role: 'assistant', content: fullReply, providerLabel, providerKey, ...(videoSrc ? { videoSrc } : {}) });
