...
export async function repairMermaid({ provider, all, model, source, errorText }) {
  ...
  await dispatchStyleStream({
    apiStyle: provider.apiStyle || 'chat',
    common,
    onDelta,
    system: SYSTEM_PROMPT,
    user,
    // agent providers (opencode/bridge) have no /chat/completions route —
    // one-shot agent turn instead of a chat-completions 404.
    agent: { provider, all },
  });
