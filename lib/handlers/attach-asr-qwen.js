...
export async function uploadBlobToQwen({ blob, filename, apiKey, baseUrl: rawBaseUrl, model, onProgress } = {}) {
  ...
  try {
    ...
    // retryable 标记网络级/5xx 失败（ERR_CONNECTION_RESET 这类瞬时故障重试有意义），
    // 4xx（签名/策略/大小问题）是确定性失败，重发也是同样结果。
    const send = (fd) => new Promise((resolve) => {
      if (onProgress && typeof XMLHttpRequest !== 'undefined') {
        ...
        xhr.onload = () => resolve(xhr.status >= 200 && xhr.status < 300
          ? { ok: true }
          : { ok: false, error: `qwen OSS upload HTTP ${xhr.status}`, retryable: xhr.status >= 500 });
        xhr.onerror = () => resolve({ ok: false, error: 'qwen OSS upload network error（连接被重置/无法建立）', retryable: true });
        xhr.onabort = () => resolve({ ok: false, error: 'qwen OSS upload aborted', retryable: false });
        xhr.send(fd);
        return;
      }
      fetch(pol.upload_host, { method: 'POST', body: fd })
        .then((res) => resolve(res.ok
          ? { ok: true }
          : { ok: false, error: `qwen OSS upload HTTP ${res.status}`, retryable: res.status >= 500 }))
        .catch(() => resolve({ ok: false, error: 'qwen OSS upload network error（连接被重置/无法建立）', retryable: true }));
    });
    ...
    // 网络级失败自动重试（连接重置对大文件上传并不罕见）；FormData 可重复发送，
    // 每次尝试重新走一遍完整 POST。间隔 1.2s / 2.4s 线性退避。
    const OSS_UPLOAD_ATTEMPTS = 3;
    let lastErr = '';
    for (let attempt = 1; attempt <= OSS_UPLOAD_ATTEMPTS; attempt++) {
      ...
      lastErr = up.error || 'unknown';
      if (!up.retryable) break;
    }
    return { ok: false, error: `${lastErr} | attempts: ${OSS_UPLOAD_ATTEMPTS} | host: ${pol.upload_host}` };
  } catch (e) {
...
function sumQwenUsage(a, b) {
  ...
  return {
    ...
    ...(a.seconds || b.seconds ? { seconds: (a.seconds || 0) + (b.seconds || 0) } : {}),
  };
}
...
// qwen-audio-3.0-asr-flash-filetrans（官方对播客/访谈场景的推荐模型，热词+Prompt
// 上下文+说话人分离）/ qwen3-asr-flash-filetrans / fun-asr 走这条管线：
//   POST /api/v1/services/audio/asr/transcription（X-DashScope-Async: enable）
//     → {output:{task_id}} → 轮询 GET /api/v1/tasks/{task_id} 到 SUCCEEDED
//     → GET output.result.transcription_url（签名 OSS JSON，24h 有效，免鉴权）
//     → {transcripts:[{sentences:[{begin_time,end_time,text,...}]}]}
// 关键：官方文档明确 RESTful 调用支持 oss:// 临时 URL——我们的 48h 私有临时文件
// 可以直接喂这条 API，不要求公网地址（SDK 调用才有此限制）。

/** 是否走录音文件转写异步任务 API（filetrans 离线专用系 + fun-asr 系）。 */
export function isQwenFiletransModel(model) {
  const m = String(model || '').trim().toLowerCase();
  return /-filetrans$/.test(m) || m.startsWith('fun-asr');
}
 ...
 * 毫秒精确；说话人字段防御性解析（speaker_id / spk_id / speaker——qwen-audio-3.0-asr
 * 官方宣称支持说话人分离，但 API 参考的 sentence 字段表尚未列出该字段名，先按常见
 * 形态接，真实响应到手后按实际字段收敛）。有说话人 → 按全片首次出现顺序编号
 * [说话人N]；解析不出 → 无标签（与单人音频不标的协议一致）。
 */
...
/**
 * filetrans 转写：提交 → 轮询 → 拉结果 → 映射。出参与 Omni chat 路径同构
 * （{ text, usage? }，永不 truncated——ASR 没有 token 上限概念）。失败全部响亮
 * 抛错（任务级失败带 code+message），不做静默回退——filetrans 提交失败说明模型
 * ID 或文件引用有问题，回退只会掩盖配置错误。
 */
async function transcribeQwenFiletrans({ baseUrl, apiKey, fileId, model, language, signal, pollIntervalMs }) {
  const apiV1 = qwenUploadOrigin(baseUrl);
  // enable_words=true：VAD+标点断句（比纯 VAD 的句子边界更贴语义）；enable_itn：
  // 数字转阿拉伯数字（字幕可读性）。language=auto 时不传，让模型自检（官方要求
  // 语种不确定时不要指定）。
  const parameters = { enable_itn: true, enable_words: true };
  if (language && language !== 'auto') parameters.language = language;
  // 说话人分离：平台 FAQ 文档化在 Fun-ASR 上；其它 filetrans 模型不发该参数
  //（防未知参数被拒），说话人靠结果 JSON 的防御性解析兜住。
  if (/^fun-asr/i.test(model)) parameters.diarization_enabled = true;
  const submit = await fetch(`${apiV1}/services/audio/asr/transcription`, {
    ...
    body: JSON.stringify({ model, input: { file_url: fileId }, parameters }),
    signal,
  ...
  try {
    ...
    if (signal?.aborted) {
      throw new Error('qwen 转写任务轮询中止：超时预算用尽（长音频转写通常 5-20 倍速于实时，重试即可）');
    }
    throw e;
  }
  const url = out?.result?.transcription_url || out?.results?.[0]?.transcription_url || '';
  if (!url) {
  ...
  return {
    text,
    ...(out?.usage?.seconds ? { usage: { input_tokens: 0, output_tokens: 0, total_tokens: 0, seconds: out.usage.seconds } } : {}),
  };
}
 ...
 */
export async function transcribeAudioQwen({ baseUrl, apiKey, fileId, model, language = 'zh', durationSec = 0, forVideoAnalysis = false, signal, idleTimeoutMs = 60_000, pollIntervalMs = 4000 }) {
  if (!fileId) throw new Error('no fileId');
  if (!model) throw new Error('no model');
  // 录音文件转写系（*-filetrans / fun-asr）走异步任务 API：句级毫秒时间戳（比
  // Omni 猜的时间戳稳得多）、价格低一个量级、官方标称最长 12 小时——长音频主力。
  // Omni chat 路径保留：用户手填 omni 系模型时的 prompt 转写（带语义行为）。
  if (isQwenFiletransModel(model)) {
    return transcribeQwenFiletrans({ baseUrl, apiKey, fileId, model, language, signal, pollIntervalMs });
  }
  const body = {
    model,
    messages: [
