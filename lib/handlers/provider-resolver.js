...
export function resolveProvider(all) {
  ...
  // Native-messaging agent providers (Codex / WorkBuddy) have NO baseUrl by
  // design — the connection is a locally-spawned engine discovered by the
  // bridge, and their real gate is the options-page Ping. A URL check here
  // would block every turn with "Base URL is not set" (real Mac test,
  // 2026-09-02).
  const isNmAgent = !!(provider.isCodex || provider.isWorkbuddy);
