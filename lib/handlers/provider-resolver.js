...
/**
 * 回复铭牌（.msg-provider）上「哪个模型/端点回了这条」的后缀值，CHAT 与
 * SUBCHAT 共用，保证主聊天和追问卡对同一选择显示同一个名字。bridge：选中
 * 的端点 URL（alias 由 providerEntryLabel 从 /health 发现表解析）；opencode/
 * Hermes：空（模型由服务端 agent 自己管，铭牌只落裸名）；LLM 卡：主页下拉
 * 选中的模型。
 */
export function resolveReplyModelOrEndpoint(provider, all, bridgeEndpoint) {
  if (provider?.isBridge) return bridgeEndpoint || provider.baseUrl || '';
  if (provider?.isOpencode || provider?.isHermes) return '';
  return resolveChatModel(provider, all) || '';
}
