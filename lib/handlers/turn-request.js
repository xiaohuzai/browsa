 ...
 * @param {object} args.builders      chat-handler-local builders, injected to
 *   avoid an import cycle (they are defined in chat-handler.js and imported by
 *   its tests): { buildHermesTurn, buildRunsConversationHistory,
 *   buildResponsesInput, buildAnthropicMessages }
 */
export function createTurnRequest({ provider, activeProvider, all, msg, sendHistory, effectiveSystemPrompt, builders }) {
  ...
  const t = {
    kind,
    // opencode/bridge keep their transcripts server-side, so stubbing the
    // resent prompt cannot shrink what the agent already stored — the
    // overflow self-rescue must not fire for them.
    overflowRescuable: kind !== 'opencode' && kind !== 'bridge',
  ...
  t.prepare = async () => {
    if (kind === 'hermes') {
      ...
      ({ input: t.runsInput, conversationHistory: t.runsConvHistory } = builders.buildHermesTurn(msg, sendHistory));
      return;
    }
    if (kind === 'opencode') {
    ...
    if (kind === 'responses') {
      t.responsesInput = builders.buildResponsesInput(msg, sendHistory);
      return;
    }
    if (kind === 'anthropic') {
      t.anthropicMessages = builders.buildAnthropicMessages(msg, sendHistory);
  ...
  t.rebuildFrom = (history) => {
    if (kind === 'hermes') {
      t.runsConvHistory = builders.buildRunsConversationHistory(history);
    } else if (kind === 'responses') {
      t.responsesInput = builders.buildResponsesInput(msg, history);
    } else if (kind === 'anthropic') {
      t.anthropicMessages = builders.buildAnthropicMessages(msg, history);
    } else if (kind === 'chat') {
      t.messages = buildMessages({
        history,
        userText: msg.userText,
  ...
  t.continueWith = (instruction, priorReply) => {
    ...
    if (kind === 'responses') {
      t.responsesInput = builders.buildResponsesInput({ userText: instruction }, contHistory);
    } else if (kind === 'anthropic') {
      t.anthropicMessages = builders.buildAnthropicMessages({ userText: instruction }, contHistory);
    } else {
      t.messages = buildMessages({
        history: contHistory,
  ...
  t.rewriteWith = (instruction, history) => {
    ...
    if (kind === 'responses') {
      t.responsesInput = builders.buildResponsesInput({ userText: instruction }, history);
    } else if (kind === 'anthropic') {
      t.anthropicMessages = builders.buildAnthropicMessages({ userText: instruction }, history);
    } else {
      t.messages = buildMessages({
        history,
        userText: instruction,
