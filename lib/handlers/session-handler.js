...
export async function handleSession(msg) {
  switch (msg.type) {
    ...
    case 'GET_SESSIONS': {
      ...
      // In-flight turn markers (2026-09-24): any live stream whose ORIGIN is
      // this session — foreground or backgrounded by a session switch.
      const origins = new Set([...streamState.values()].map((st) => st.originSessionId || '').filter(Boolean));
      return { sessions: sessions.map((s) => (origins.has(s.id) ? { ...s, active: true } : s)) };
    }

    case 'LOAD_SESSION': {
      // len === -1 表示会话不存在（storage.loadSession 的约定）；0 是合法的空会话。
