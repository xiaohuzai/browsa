    ...
    try {
      ...
      if (provider.isOpencode) {
        ...
        if (!subSessionId) {
          ...
          subchatOpencodeSessions.set(subId, subSessionId);
        }
        ...
        // models slot holds endpoint URLs, activeModel picks one.
        ...
        st = await bridgeStream({
          ...
          onDelta,
          signal: controller.signal,
          onSessionId: (sid) => subchatBridgeSessions.set(subId, sid),
        });
