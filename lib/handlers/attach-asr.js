  ...
  const body = {
    ...
    input: [{
      ...
      content: [
        ...
        { type: 'input_text', text: '请逐字转写这段音频的语音内容：每句一行、每行行首加一个单一起始时刻时间戳 [mm:ss]（超过一小时用 [h:mm:ss]），不要输出区间（如 [00:00-00:12]）也不要小数（如 [00:12.5]），括号内只放这一个起始时间；必须补全中文标点（，。！？），即使原音频没有明显停顿也要按语义断句，不要输出一整段没有标点的长句；说话人标签规则：每一个独立人声都算一个说话人——主讲人/旁白、插播的采访或演讲录音、画外音、电话音、换了语言的其他人都算。只有一个人说话全程才不标；否则每一行都必须在时间戳后标注 [说话人N]（主讲人也不例外，无例外），编号从 1 开始按全片首次出现的声音顺序分配，同一个声音全片用同一个号，不能把两个人合并成一个号，也不能把一个人拆成两个号。' }
      ]
    }],
    stream: true,
...
async function streamResponsesText({ baseUrl, apiKey, body, signal, idleTimeoutMs = 60_000 }) {
  ...
  console.log(`[ASR] transcribe: SSE stream ended (server closed cleanly) chars=${String(full || '').length} truncated=${truncated} finishReason=${finishReason || 'none'}`);
  return {
...
export function normalizeAsrTimestamps(text) {
  ...
  return String(text).replace(TOKEN_RE, (whole, h, m, s) => `[${fmt(toSec(h, m, s))}]`);
}
...
export function transcriptEndSec(rawText) {
  ...
  for (const line of String(rawText).split(/\r?\n/)) {
    ...
    while ((m = TS_RE.exec(line)) !== null) {
      // 有区间右端（组 5-8 任一命中）就用右端；否则用单时间戳（组 1-4）。
      // 注意不能只判 m[5]（右端小时）：普通分钟级区间 [33:17.22-33:25.45] 的
      // 右端小时是空的，只看 m[5] 会把区间误当单时间戳、返回左端时刻。
      const hasRange = m[5] != null || m[6] != null || m[7] != null || m[8] != null;
      const end = hasRange ? toSec(m[5], m[6], m[7], m[8]) : toSec(m[1], m[2], m[3], m[4]);
...
export function buildVideoAnalysisInstructions(language = 'zh', durationSec = 0) {
  ...
  const cap = keyframeCapFor(durationSec);
  return 'You are producing a timestamped AUDIOVISUAL READING document (视听精读) for a video. ' +
    ...
    'compress it into ONE line "[mm:ss] （广告：brand + core pitch）" and never place a keyframe marker inside it; ' +
    'content that merely discusses a product as part of the topic is NOT an ad. ' +
    'KEYFRAME SCREENSHOT MARKERS (up to ' + cap + '): when a 「画面：」 moment carries information whose VISUAL is independently valuable ' +
    '(a chart or data figure, a slide, on-screen code or terminal output, a key demonstration step), immediately AFTER that 「画面：」 line ' +
    'add ONE marker line of its own: "[mm:ss] [截屏] 短标题" — the same timestamp as that moment, followed by a short noun-phrase title. ' +
    'Treat ' + cap + ' as a budget to spend on chart/slide-dense videos (roughly one every two minutes), at least 10 seconds apart, spread across the video; ' +
    'when nothing qualifies, output NO markers. ' +
    'Markers are additional lines, never replacements. ' +
    'PUNCTUATION IS MANDATORY: complete, readable sentences; one sentence or one bullet per line; break long speech at semantic boundaries. ' +
    ...
    'Never merge two people into one number and never split one person into two. ' +
    'When a speaker\'s identity is evident from the video (a name card, an on-screen caption, an unmistakable public figure), ' +
    'append the name in parentheses at the end of that speaker\'s FIRST line — e.g. "……（特朗普）". ' +
 ...
 */
export function buildVideoAnalysisTaskText(durationSec = 0, language = 'zh') {
  ...
  return '请结合画面与声音，逐段产出这份视频的「视听精读」文档：语音内容忠实转写（术语、人名、数字必须准确）；' +
    ...
    '关键帧截图标记（上限 ' + cap + ' 个）：当某条「画面：」描述的画面具有独立信息价值（图表、幻灯片、屏幕代码或终端输出、关键演示步骤）时，' +
    '紧随其后单独加一行 `[mm:ss] [截屏] 短标题`（时间与该画面一致，标题用简短名词短语）。' +
    '把 ' + cap + ' 当作预算尽量用满——图表/幻灯片密集的视频约每 2 分钟一处，彼此至少间隔 10 秒，尽量分布均匀；没有值得截图的画面就不要输出任何标记。' +
    '每行行首加一个单一起始时刻时间戳 [mm:ss]（超过一小时用 [h:mm:ss]），括号内只放这一个起始时间，不要区间也不要小数；' +
    '必须补全标点，一句一行。说话人标签规则：每一个独立人声都算一个说话人——主讲人/旁白、插播的采访或演讲录音、画外音、电话音、换了语言的其他人都算；' +
    '只有一个人说话全程才不标，否则每一行都在时间戳后标注 [说话人N]（主讲人也不例外），' +
    '编号按全片首次出现的顺序从 1 开始分配，同一个声音全片用同一个号，不能合并也不能拆分；' +
    '说话人身份能从画面确认时（姓名条、字幕条、公众人物），在该说话人第一行末尾括注姓名，如「……（特朗普）」。' + durHint +
    '从开头覆盖到结尾，不要总结、不要开场白、不要代码块围栏。';
}

/**
 * 截屏标记上限随时长伸缩：约每 2 分钟一个名额，4~12 封顶；时长未知回退 6
 * （历史默认值）。prompt 里的上限与 sidepanel 调 parseKeyframeMarkers 传的 max
 * 必须同源（都走本函数）——否则客户端会把模型按 prompt 合法产出的标记静默砍掉。
 * 密度依据（2026-08-30 用户反馈）：16:55 的视频 6 张封顶被用户质疑「固定就6张」
 * ——cap 是上限不是指标，模型偏保守就用不满；放宽到 d/120 并在 prompt 里明确
 * 「图表密集的视频请尽量用满额度」。
 */
export function keyframeCapFor(durationSec) {
  const d = Number(durationSec) || 0;
  if (d <= 0) return 6;
  return Math.min(12, Math.max(4, Math.ceil(d / 120)));
}
 ...
 */
export async function analyzeVideo({ baseUrl: rawBaseUrl, apiKey, videoFileId, audioFileId, model, language = 'zh', durationSec = 0, signal, idleTimeoutMs = 60_000 }) {
  const baseUrl = normalizeArkBaseUrl(rawBaseUrl);
  ...
  content.push({ type: 'input_text', text: buildVideoAnalysisTaskText(durationSec, language) });
  const body = {
    model,
