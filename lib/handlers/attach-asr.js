...
//
// ASR / 视听精读的服务商适配层：火山方舟（Volcengine Ark）适配器 + 多供应商注册表
// （千问AI平台见 attach-asr-qwen.js）。sidepanel 管线经 asrAdapterFor(provider) 分发，
// 上传/轮询/转写/精读对供应商无感知。
//
...
// 千问AI平台适配器（协议见 attach-asr-qwen.js 头注）。注册进下方 ASR_ADAPTERS，
// sidepanel 经 asrAdapterFor(provider) 分发——两条管线对供应商无感知。
import {
  normalizeQwenBaseUrl, uploadBlobToQwen, pollFileStatusQwen, transcribeAudioQwen, analyzeVideoQwen,
...
// ─── ASR 文件复用缓存（方舟 Files / 千问临时文件通用）──────────────────────────
// 两家的官方定位都支持「同一文件多次请求复用，避免重复上传」：方舟 file_id 30 天、
// 千问 oss:// 临时 URL 48 小时。这里把上传成功的引用按「供应商 + baseUrl + key 指纹 +
// 视频资产标识」存 chrome.storage.local，同一视频有效期内再解析直接复用，免掉最慢
// 最容易失败的上传段（音频模式连下载+转码一起免掉）。任何一环不可用（storage 缺失 /
// entry 损坏 / 文件过期 / 千问的模型绑定不符）都 fail-open 回退完整上传流程，绝不因
// 缓存引入新故障。
//
// 千问特有约束：临时文件与【上传时指定的 model】绑定，换模型旧文件一律不可用——
// 所以 entry 里逐文件记录绑定模型（videoModel/audioModel），lookup 严格校验；且平台
// 不提供查询接口，无法探活，只按 48 小时 TTL 判活（方舟照旧 GET /files/{id} 探活）。

const ASR_FILE_CACHE_STORE_KEY = 'browsaArkFileCache'; // 存储键沿用旧值：升级不清掉已存的方舟缓存
const QWEN_FILE_TTL_SEC = 48 * 3600;
...
/**
 * 缓存 key：供应商命名空间 + 规整后的 baseUrl + apiKey 指纹 + 资产标识（file_id 不跨
 * 账号/网关通用）。方舟保持旧格式（不含供应商段）——升级后已存的 30 天缓存继续命中；
 * 千问加 `qwen|` 前缀与独立 base 规整。模型不进 key：绑定模型记录在 entry 逐文件校验，
 * 否则同一条目里的视频/音频（分别绑定 videoModel/transcribeModel）没法共存。
 */
export function asrFileCacheKey(provider, baseUrl, apiKey, assetId) {
  const fp = apiKey ? `${apiKey.length}:${apiKey.slice(-4)}` : 'nokey';
  if (provider === 'qwen') {
    return `qwen|${normalizeQwenBaseUrl(baseUrl)}|${fp}|${assetId}`;
  }
  return `${normalizeArkBaseUrl(baseUrl)}|${fp}|${assetId}`;
}

/** 方舟旧 key 构造（保留给既有调用方/测试；等价 asrFileCacheKey('ark', …)）。 */
export function arkFileCacheKey(baseUrl, apiKey, assetId) {
  return asrFileCacheKey('ark', baseUrl, apiKey, assetId);
}
...
/**
 * 查缓存（多供应商）。返回 { videoFileId, audioFileId }（只含未过期、通过探活/
 * 模型绑定校验的字段；调用方按字段独立决定跳过哪些步骤），无可用条目返回 null。
 * need: 'video' | 'audio' 仅决定必查哪个字段，另一字段命中也照常返回（音频模式先跑
 * 过、视频模式可复用其音频）。千问：按 48h TTL 判活 + entry 里的绑定模型严格匹配，
 * 无探活（平台没有查询接口）；方舟：30d expireAt + GET /files/{id} 探活。
 */
export async function lookupCachedAsrFiles({
  provider = 'ark', baseUrl, apiKey, platform, pageUrl, need = 'video', durationSec = 0,
  videoModel = '', audioModel = '', storageArea, aliveFn,
} = {}) {
  ...
  let entry = null;
  try {
    entry = (await area.get(ASR_FILE_CACHE_STORE_KEY))?.[ASR_FILE_CACHE_STORE_KEY]?.[asrFileCacheKey(provider, baseUrl, apiKey, assetId)] || null;
  } catch (_) {
    return null;
  }
  ...
  // 千问的临时文件与上传时的 model 绑定：换模型（或 entry 没记绑定模型）→ 该文件不可用
  const bindOk = (want, got) => !isQwen || (!!got && got === want);
  const probe = aliveFn ?? (isQwen ? null : arkFileAlive);
  const out = { videoFileId: '', audioFileId: '' };
  if (fresh(entry.videoFileId, entry.videoExpireAt)
    && bindOk(videoModel, entry.videoModel)
    && (!probe || await probe({ baseUrl, apiKey, fileId: entry.videoFileId }))) {
    out.videoFileId = entry.videoFileId;
  }
  if (fresh(entry.audioFileId, entry.audioExpireAt)
    && bindOk(audioModel, entry.audioModel)
    && (!probe || await probe({ baseUrl, apiKey, fileId: entry.audioFileId }))) {
...
/**
 * 写缓存（上传 + 处理成功后调用）。video/audio 字段按本次实际上传的合并——视频模式
 * 只传视频时保留旧的音频字段，反之亦然（两次不同模式各自积累）。千问逐文件记录绑定
 * 模型、TTL 48 小时；方舟 expireAt 30 天上限。存储不可用静默放弃。
 */
export async function saveAsrFileCacheEntry({
  provider = 'ark', baseUrl, apiKey, platform, pageUrl,
  videoFileId = '', audioFileId = '', videoModel = '', audioModel = '',
  durationSec = 0, storageArea, expireAtSec,
} = {}) {
  const isQwen = provider === 'qwen';
  const assetId = videoAssetId(platform, pageUrl);
  ...
  const area = arkCacheStorageArea(storageArea);
  if (!area) return;
  const key = asrFileCacheKey(provider, baseUrl, apiKey, assetId);
  let map = {};
  try {
    map = (await area.get(ASR_FILE_CACHE_STORE_KEY))?.[ASR_FILE_CACHE_STORE_KEY] || {};
  } catch (_) { /* 读失败按空 map 处理，写失败下方 catch */ }
  const prev = map[key] || {};
  const exp = expireAtSec ?? (isQwen ? Math.floor(Date.now() / 1000) + QWEN_FILE_TTL_SEC : arkExpireAtSec());
  map[key] = {
    videoFileId: videoFileId || prev.videoFileId || '',
    videoExpireAt: videoFileId ? exp : (prev.videoExpireAt || 0),
    videoModel: isQwen ? (videoFileId ? videoModel : (prev.videoModel || '')) : (prev.videoModel || ''),
    audioFileId: audioFileId || prev.audioFileId || '',
    audioExpireAt: audioFileId ? exp : (prev.audioExpireAt || 0),
    audioModel: isQwen ? (audioFileId ? audioModel : (prev.audioModel || '')) : (prev.audioModel || ''),
    durationSec: durationSec || prev.durationSec || 0,
    updatedAt: Math.floor(Date.now() / 1000),
  };
  try {
    await area.set({ [ASR_FILE_CACHE_STORE_KEY]: map });
  } catch (_) { /* 缓存写失败不影响主流程 */ }
}
...
/** 全部时间戳端点（秒，升序去重）。transcriptEndSec 的旧语义（最大端点）。 */
export function collectTranscriptStamps(rawText) {
  const stamps = new Set();
  for (const [a, b] of parseTranscriptTimeline(rawText)) {
    stamps.add(a);
    stamps.add(b);
  }
...
// ─── 服务商适配器注册表 ────────────────────────────────────────────────────────
// 统一签名：{ transcribeAudio, analyzeVideo, uploadBlob, pollFileStatus }。入参
// { baseUrl, apiKey, fileId..., model, language, signal, idleTimeoutMs }，出参
// { text, truncated?, finishReason? }。uploadBlob 返回 {ok, fileId, bytes}（fileId 是
// 供应商无关的文件引用：方舟 file_id / 千问 oss:// 临时 URL），pollFileStatus 返回
// { status, ready, error? }。新供应商（百炼/GLM…）= 在 ASR_PROVIDERS 注册表加元数据 +
// 这里注册一个同签名适配器，sidepanel 经 asrAdapterFor(asr.provider) 分发，UI 与管线
// 零改动。未知 id 回退 ark（老配置无 provider 字段时同样成立）。
const ASR_ADAPTERS = {
  ark: { transcribeAudio, analyzeVideo, uploadBlob: uploadBlobToArk, pollFileStatus },
  qwen: {
    transcribeAudio: transcribeAudioQwen,
    analyzeVideo: analyzeVideoQwen,
    uploadBlob: uploadBlobToQwen,
    pollFileStatus: pollFileStatusQwen,
  },
};
