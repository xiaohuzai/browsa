 ...
 *
 * @param {Object} opts  { audioUrl }
 * @returns {Promise<{ok:true, blob:Blob, bytes:number} | {ok:false, error:string}>}
 */
export async function downloadAudioBytes({ audioUrl } = {}) {
...
export async function transcodeAudioBlob(blob, { targetSampleRate = TRANSCODE_SAMPLE_RATE, channels = TRANSCODE_CHANNELS } = {}) {
  try {
    ...
    // 手动降采样到目标采样率 + 多声道→单声道混音（纯函数，Node 可测）。
    const out = resampleToMono(decoded, targetSampleRate);
 ...
 */
export async function uploadBlobToArk({ blob, filename, mime, apiKey, baseUrl: rawBaseUrl } = {}) {
...
export async function transcribeAudio({ baseUrl: rawBaseUrl, apiKey, fileId, model, language = 'zh', signal, idleTimeoutMs = 60_000 }) {
  ...
  const body = {
    model,
    instructions:
      'You are an automatic speech recognition (ASR) engine with sentence-level punctuation ' +
      'and speaker diarization. Transcribe the audio verbatim in its original language. ' +
      `${langHint} ` +
      'Output requirements: ' +
      '(1) Add proper punctuation (，。！？etc.) so every sentence is a complete, readable sentence — ' +
      'do NOT output run-on unpunctuated text. ' +
      '(2) Put ONE sentence per line — never merge multiple sentences into one long block. ' +
      '(3) Start every line with the sentence\'s start time as [mm:ss] (e.g. [01:23]); for timestamps ' +
      'over an hour use [h:mm:ss]. ' +
      '(4) If you detect MULTIPLE distinct speakers, prefix each line with a speaker label right ' +
      'after the timestamp, e.g. "[00:12] [说话人1] 你好" or "[00:15] [Speaker 2] Hello". ' +
      'If the audio is a single speaker (monologue), do NOT add speaker labels at all. ' +
      '(5) Preserve the exact wording and order — transcribe verbatim, never summarize, ' +
      'paraphrase, or omit content. ' +
      'Output ONLY the transcript lines — no preamble, no summaries, no markdown fences, no numbering.',
    input: [{
      role: 'user',
      content: [
        { type: 'input_audio', file_id: fileId },
        { type: 'input_text', text: '请逐字转写这段音频的语音内容，每句一行，补全标点，行首加 [mm:ss] 时间戳；如有多位说话人，在时间戳后标注 [说话人1]/[说话人2]。' }
      ]
    }],
    stream: true,
  };
