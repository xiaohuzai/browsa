...
export function buildVideoAnalysisInstructions(language = 'zh') {
  ...
  return 'You are producing a timestamped AUDIOVISUAL READING document (视听精读) for a video. ' +
    ...
    'KEYFRAME SCREENSHOT MARKERS: mark EVERY 「画面：」 moment whose VISUAL is independently valuable ' +
    '(a chart or data figure, a slide, on-screen code or terminal output, a key demonstration step) — as many as the content warrants, ' +
    'a chart/slide-dense video may need one every minute or more; never skip a valuable frame for brevity. ' +
    'Each marker is ONE line of its own immediately AFTER that 「画面：」 line: "[mm:ss] [截屏] 短标题" — the same timestamp as that moment, ' +
    'followed by a short noun-phrase title. At least 10 seconds apart (skip near-identical consecutive frames); ' +
...
export function buildVideoAnalysisTaskText(durationSec = 0, language = 'zh') {
  ...
  return '请结合画面与声音，逐段产出这份视频的「视听精读」文档：语音内容忠实转写（术语、人名、数字必须准确）；' +
    ...
    '关键帧截图标记：有多少值得截的就标多少——只要某条「画面：」描述的画面具有独立信息价值（图表、幻灯片、屏幕代码或终端输出、关键演示步骤），' +
    '就紧随其后单独加一行 `[mm:ss] [截屏] 短标题`（时间与该画面一致，标题用简短名词短语）；' +
    '图表/幻灯片密集的视频可能每分钟都需要一处，不要为了精简跳过有价值的画面；彼此至少间隔 10 秒，近似的连续帧跳过。' +
