...
export function buildVideoAnalysisInstructions(language = 'zh', durationSec = 0) {
  ...
  return 'You are producing a timestamped AUDIOVISUAL READING document (视听精读) for a video. ' +
    ...
    'KEYFRAME SCREENSHOT MARKERS (at most ' + cap + '): when a 「画面：」 moment carries information whose VISUAL is independently valuable ' +
    ...
    'At most ' + cap + ' markers in the ENTIRE document, at least 10 seconds apart, spread across the video; when nothing qualifies, output NO markers. ' +
...
export function buildVideoAnalysisTaskText(durationSec = 0, language = 'zh') {
  ...
  return '请结合画面与声音，逐段产出这份视频的「视听精读」文档：语音内容忠实转写（术语、人名、数字必须准确）；' +
    ...
    '关键帧截图标记（最多 ' + cap + ' 个）：当某条「画面：」描述的画面具有独立信息价值（图表、幻灯片、屏幕代码或终端输出、关键演示步骤）时，' +
    '紧随其后单独加一行 `[mm:ss] [截屏] 短标题`（时间与该画面一致，标题用简短名词短语）。' +
    '全片最多 ' + cap + ' 个，彼此至少间隔 10 秒，尽量分布均匀；没有值得截图的画面就不要输出任何标记。' +
...
/**
 * 截屏标记上限随时长伸缩：约每 2.5 分钟一个名额，4~10 封顶；时长未知回退 6
...
export function keyframeCapFor(durationSec) {
  ...
  if (d <= 0) return 6;
  return Math.min(10, Math.max(4, Math.ceil(d / 150)));
}
