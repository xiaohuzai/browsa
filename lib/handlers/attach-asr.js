...
export function transcriptEndSec(rawText) {
  ...
  // 裸秒数兜底（2026-08-30 真实故障：精读输出退化成 [0.0]/[624.0] 原生秒级时间轴，
  // 上面那个 mm:ss 正则一行都匹配不到 → 返回 null → 完整度守卫被静默跳过）。
  // 只认【行首】纯数字（可带小数，保留小数精度——不能复用归一化的截断版，边界
  // 90% 校验会误判）。>12h 视为非时间戳。
  ...
  for (const line of String(rawText).split(/\r?\n/)) {
    ...
    if (!matched) {
      ...
      if (bare) {
        ...
        if (total <= 43200 && (maxEnd == null || total > maxEnd)) maxEnd = total;
      }
    }
  }
  return maxEnd;
