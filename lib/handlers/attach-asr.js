...
export async function transcribeAudio({ baseUrl: rawBaseUrl, apiKey, fileId, model, language = 'zh', signal, idleTimeoutMs = 60_000 }) {
  ...
  const body = {
    ...
    instructions:
      ...
      'SPEAKER LABELS (deterministic rules): if the ENTIRE audio has only one speaker, output NO labels at all. ' +
      'Otherwise this audio has MULTIPLE distinct speakers — label EVERY line EXACTLY: each line starts with ' +
      '"[mm:ss] [说话人N] text" (the label sits right after the timecode, with NO exception — this includes ' +
      'the narrator/host/interviewer, whose lines must also carry their own label). ' +
    ...
    input: [{
      ...
      content: [
        ...
        { type: 'input_text', text: '请逐字转写这段音频的语音内容：每句一行、每行行首加一个单一起始时刻时间戳 [mm:ss]（超过一小时用 [h:mm:ss]），不要输出区间（如 [00:00-00:12]）也不要小数（如 [00:12.5]），括号内只放这一个起始时间；必须补全中文标点（，。！？），即使原音频没有明显停顿也要按语义断句，不要输出一整段没有标点的长句；说话人标签规则：如果全片只有一个人说话就不标；否则每一行都必须在时间戳后标注 [说话人N]（包括主持人/旁白/采访者，无例外），编号从 1 开始按全片首次出现的声音顺序分配，同一个声音全片用同一个号，不能把两个人合并成一个号，也不能把一个人拆成两个号。' }
      ]
    }],
    stream: true,
...
export function buildVideoAnalysisInstructions(language = 'zh') {
  ...
  return 'You are producing a timestamped AUDIOVISUAL READING document (视听精读) for a video. ' +
    ...
    'KEYFRAME SCREENSHOT MARKERS (at most 6): when a 「画面：」 moment carries information whose VISUAL is independently valuable ' +
    ...
    'At most 6 markers in the ENTIRE document, at least 10 seconds apart, spread across the video; when nothing qualifies, output NO markers. ' +
...
export function buildVideoAnalysisTaskText(durationSec = 0, language = 'zh') {
  ...
  return '请结合画面与声音，逐段产出这份视频的「视听精读」文档：语音内容忠实转写（术语、人名、数字必须准确）；' +
    ...
    '关键帧截图标记（最多 6 个）：当某条「画面：」描述的画面具有独立信息价值（图表、幻灯片、屏幕代码或终端输出、关键演示步骤）时，' +
    '紧随其后单独加一行 `[mm:ss] [截屏] 短标题`（时间与该画面一致，标题用简短名词短语）。' +
    '全片最多 6 个，彼此至少间隔 10 秒，尽量分布均匀；没有值得截图的画面就不要输出任何标记。' +
    '每行行首加一个单一起始时刻时间戳 [mm:ss]（超过一小时用 [h:mm:ss]），括号内只放这一个起始时间，不要区间也不要小数；' +
    '必须补全标点，一句一行。说话人标签规则：全片只有一个人说话就不标；否则每一行都在时间戳后标注 [说话人N]，' +
    '编号按全片首次出现的顺序从 1 开始分配，同一个声音全片用同一个号。' + durHint +
