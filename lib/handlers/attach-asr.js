...
export function splitMp4Fragments(arrayBuffer) {
  try {
    ...
    if (!sawMoof || fragments.length === 0) return null;
  ...
  try {
    ...
    try {
      try {
        ...
        // 整文件解码失败：很可能是超长音频（100+min 解码后 PCM 达 2GB+，超过
        // decodeAudioData 的单次内存上限 → “Unable to decode audio data”）。
        // 若是 fMP4 则分片解码（每片几秒，独立可解码），绕过单次解码内存上限。
        const frag = splitMp4Fragments(arrayBuffer);
        if (!frag) throw wholeErr; // 非分片无法拆 → 原样抛回
        try {
          out = await decodeFragmentsToMono(decodeCtx, frag, targetSampleRate);
        } catch (fragErr) {
          // 分片解码也失败（真编码不支持，如 Edge 上 HE-AAC）→ 把两条原因都带上
          throw new Error('decode failed (whole: ' + String((wholeErr && wholeErr.message) || wholeErr) + '; fragmented: ' + String((fragErr && fragErr.message) || fragErr) + ')');
        }
      }
    } finally {
