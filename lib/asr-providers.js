...
export const ASR_PROVIDERS = {
  ...
  qwen: {
    id: 'qwen',
    label: '千问AI平台',
    // OpenAI 兼容端点；临时文件上传/异步任务接口自动走同域 /api/v1
    defaultBaseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
    // 转写模型默认官方对播客/访谈场景推荐的录音文件转写系（热词+Prompt上下文+
    // 说话人分离，句级毫秒时间戳，价格比 Omni 低一个量级，最长 12h）；视觉系
    // （qwen3.8-flash）不吃音频，所以视频精读 = 本模型出转写主干 + 视觉模型看
    // 画面（详见 lib/handlers/attach-asr-qwen.js）。手填 omni 系（qwen3.5-omni-
    // flash 等）则走 chat/completions 的 prompt 转写。
