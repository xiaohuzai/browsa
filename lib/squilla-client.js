...
// (delta stream) → sessions.send (accepted → task_id) → session.event.* frames →
...
// runsApiStream uses.
//
// Restored verbatim from the 2026-08-31 integration (removed 2026-09-03 while
// the upstream origin-guard PR was unmerged; upstream has since merged it —
// see AGENTS.md "OpenSquilla agent provider"). The wire contract below is
// verified against gateway 0.5.4.
...
const SEND_TIMEOUT_MS = 20_000;      // sessions.send / subscribe res budget
...
export async function squillaStream({
  ...
  try {
    ...
    // Subscribe to the session's message stream BEFORE the send so no delta
