...
//                                  (`agent` is the bridge's own name for the
//                                  agent it fronts — browsa shows it as the
//                                  alias in the provider dropdown; serve mode
//                                  runs one bridge PER agent, each on its own
//                                  port, so multi-agent = multiple endpoints)
//   POST /turns {text,sessionId,images?}
//                                → SSE: start/delta/tool/approval/usage/
//                                  done/aborted/error events (": ka" comments
//                                  are keepalives). images: ≤8 https:/data:
//                                  URL strings riding through to the agent;
//                                  the daemon's 4MB JSON body cap bounds them
//                                  (browsa applies the shared caps in
//                                  agent-turn.js's pickTurnImages first).
...
import { isAgentImageUrl } from './agent-turn.js';
...
export async function bridgeStream({
  ...
  // Defensive re-filter (the daemon 400s the whole turn on any bad element):
  // shape-strict URLs only (shared predicate), hard cap 8. The handler-side
  // withTurnImages has already applied the size budget.
