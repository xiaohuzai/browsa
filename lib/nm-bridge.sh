#!/usr/bin/env bash
# browsa ⇄ local agent engine native-messaging bridge (macOS / Linux).
#
# ONE generic bridge serves every browsa agent backend; the generated install
...
#   #__BRIDGE_BIN_OVERRIDE__ (e.g. export BROWSA_BRIDGE_BIN='/usr/local/bin/codex')
#
...
#
# Binary discovery order: BROWSA_BRIDGE_BIN → <bin> on PATH (extended with the
...
NM_LOG="${TMPDIR:-/tmp}/browsa-bridge-${BRIDGE_BIN_NAME}.log"

if [ -f "$HOME/.browsa-bridge.env" ]; then
  while IFS= read -r ev; do
    case "$ev" in ''|'#'*) continue ;; esac
    export "$ev"
  done <"$HOME/.browsa-bridge.env"
fi
...
ENGINE_BIN=""
if [ -n "${BROWSA_BRIDGE_BIN:-}" ] && [ -x "$BROWSA_BRIDGE_BIN" ]; then
  ENGINE_BIN="$BROWSA_BRIDGE_BIN"
else
...
if [ -z "$ENGINE_BIN" ]; then
  emit_error engine-binary-not-found \
    "没找到 ${BRIDGE_BIN_NAME}。装对应的 CLI 或桌面 app 任一即可；也可在 browsa 设置里指定路径后重新生成安装命令。"
  exit 1
fi
...
CONFIG=$(read_frame_text) || { emit_error bridge-no-control-frame '桥接控制帧缺失（browsa 客户端版本过旧？）'; exit 1; }
EXTRA_ARGS=""
...
BRIDGE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/browsa-bridge.XXXXXX")
