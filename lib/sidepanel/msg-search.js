...
import { $, isImeComposing, smoothOrAuto } from './ui-utils.js';
...
export function initMsgSearch() {
  ...
  // Debounced: doMsgSearch is two full-tree passes (clearHighlights+normalize,
  // then TreeWalker) over every bubble — running it per keystroke on a long
  // conversation wastes whole-tree DOM work on intermediate queries nobody
  // sees. 120ms trailing keeps it feeling instant; Enter still cycles the
  // matches of the last committed query.
  input.addEventListener('input', () => {
    clearTimeout(_searchDebounce);
    _searchDebounce = setTimeout(() => doMsgSearch(input.value), 120);
  });
...
export function closeMsgSearch() {
  ...
  if (!bar) return;
  bar.hidden = true;
  clearTimeout(_searchDebounce); // a queued keystroke search must not re-mark after close
...
function highlightSearchMatch(i) {
  ...
  if (_searchMatches[i]) {
    // behavior 走 smoothOrAuto：系统「减少动态效果」时平滑滚动降级为 auto
    // （CSS 的 media query 管不到 JS 的 behavior 参数）。
    _searchMatches[i].scrollIntoView({ block: 'center', behavior: smoothOrAuto() });
    const countEl = $('msg-search-count');
