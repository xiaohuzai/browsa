...
// True when a history blob carries evidence of an attached PDF: the
// no-extraction placeholder (`[PDF file — agent should fetch and read
// directly]`) or a `Mode: pdf` page-context header (buildPageContextText).
// Gates the pdf-inspector WASM warm-up (sidepanel init) so a panel that
// never touches PDFs never pays the 4.84MB compile. Exported for tests.
export function hasPdfTrace(text) {
  return typeof text === 'string' && /\[PDF file|Mode: pdf\b/.test(text);
}
