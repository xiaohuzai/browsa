...
self.addEventListener('message', async (ev) => {
  const { bytes, filename, type, wasmUrl, requestId } = ev.data || {};
  ...
  try {
    ...
    // Every reply echoes the request's requestId verbatim — the client
    // matches replies to pending requests BY ID and drops any reply whose id
    // has no pending entry (a timed-out request's late reply must never land
    // on the next queued request). Never reply without it. Mirrors
    // pdf-inspector.worker.js exactly.
    self.postMessage({ ok: true, markdown, requestId });
  } catch (e) {
    self.postMessage({ ok: false, error: String(e?.message ?? e), requestId });
  }
});
