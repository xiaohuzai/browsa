...
export async function renderSessionsList() {
  ...
  if (!sessions.length) {
    const msg = (key, fallback) => (typeof chrome !== 'undefined' && chrome.i18n && chrome.i18n.getMessage(key)) || fallback;
    listEl.innerHTML = `<div class="sessions-empty">${q ? msg('sessionsNoMatch', 'No sessions match your search.') : `${msg('sessionsEmpty', 'No saved sessions yet.')}<br>${msg('sessionsEmptyHint', 'Start a new session to archive this conversation.')}`}</div>`;
    return;
  }
