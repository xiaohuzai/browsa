...
import { getActiveSessionId, setActiveSessionId, getHistory } from '../storage.js';
import { ICONS } from './icons.js';
import { $, escM, showToast, showConfirmDialog, sendMessage, isImeComposing } from './ui-utils.js';

let _deps = {
  isStreaming: () => false, stopWatchingStream: () => {}, getTabId: () => null, resumeInFlight: () => {},
  renderHistory: async () => {}, scrollToBottom: () => {}, clearPendingImages: () => {},
};
...
function relativeTime(ts) {
  ...
  if (s < 60) return _msg('relJustNow', 'just now');
  // 相对时间走 Intl（U11）：英文字面模板让中文界面也显示 "3h ago"。
...
export function openSessionsDrawer() {
  ...
  renderSessionsList();
  // The drawer is role=dialog; move focus into it so keyboard users aren't left
  // tabbing through the obscured page behind it.
  setTimeout(() => { el.querySelector('.sessions-search')?.focus(); }, 0);
}
...
function createSessionRow(s) {
  ...
  item.innerHTML = `
      <div class="session-item-body">
        <div class="session-item-name" title="${escM(_msg('dblClickRename', 'Double-click to rename'))}">${escM(s.name)}</div>
        <div class="session-item-date" title="${escM(absTime)}">${relTime}${s.contentMatch ? `<span class="session-content-hit" title="${escM(_msg('contentMatchHint', 'matched in message content'))}"></span>` : ''}${s.active ? `<span class="session-active-dot" title="${escM(_msg('sessionActiveDot', '进行中'))}"></span>` : ''}</div>
      </div>
      <div class="session-item-actions">
        ...
        <button class="session-export-btn" title="${escM(_msg('exportMarkdown', 'Export as Markdown'))}" data-id="${s.id}">⬇</button>
  ...
  const bodyEl = item.querySelector('.session-item-body');
  // 键盘可达（U1）：行主体此前是 div+click/dblclick，键盘用户无法加载或
  // 重命名任何会话。Enter/Space=加载（同点击），F2=重命名（同双击）。
  bodyEl.tabIndex = 0;
  ...
  bodyEl.addEventListener('click', (e) => {
    // 编辑控件内的点击不是「选择会话」——重命名输入框里点一下调整光标，
    // 否则会冒泡到这里的 else 分支立刻 loadSession，把进行中的重命名丢掉。
    if (e.target.closest('input, textarea, select, button')) return;
  ...
  if (delBtn) {
    ...
    delBtn.addEventListener('click', async (e) => {
      ...
      showToast(_msg('sessionDeleted', 'Session deleted'), 'success');
      renderSessionsList();
    });
  }
  return item;
...
function startSessionRename(nameEl, sessionId) {
  ...
  const commit = async () => {
    ...
    if (newName !== oldName) {
      ...
      showToast(_msg('sessionRenamed', 'Session renamed'), 'success');
    }
    renderSessionsList();
  };
  const cancel = () => {
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && !isImeComposing(e)) { e.preventDefault(); commit(); }
...
async function exportSession(id, name) {
  ...
  if (!session) { showToast(_msg('sessionExportFailed', 'Could not load session'), 'error'); return; }
  ...
  showToast(_msg('sessionExported', 'Session exported'), 'success');
}

/** Clear all saved sessions with confirmation. */
export async function clearAllSessions() {
  const ok = await showConfirmDialog({
    title: _msg('clearAllTitle', 'Clear all sessions'),
    message: _msg('clearAllMsg', 'Delete all saved sessions? This cannot be undone.'),
    confirmLabel: _msg('clearAllConfirm', 'Clear all'),
    danger: true
  });
  if (!ok) return;
  await sendMessage({ type: 'CLEAR_ALL_SESSIONS' });
  showToast(_msg('sessionsCleared', 'All sessions cleared'), 'success');
  renderSessionsList();
}

export async function loadSession(id, name) {
  // Auto-save current conversation before switching. 会话归属：当前对话若已
  // 归属于某个保存的会话（activeSessionId），带上 id 让后台原地写回——否则
  // 每点一次历史清单就会 fork 一份副本，列表越点越多；只有全新对话才归档
  // 为新条目（id 缺省时的原行为）。
  //
  // 在途回复不取消（2026-09-24，用户报告「AI 还在 thinking 一切换就全没了」）：
  // 转后台 + 停止观看，回复完成后按来源会话写回它的快照（chat-handler 的
  // persistTurnEntry）。此前切换即取消，thinking 烧几分钟的回复直接蒸发。
  const history = await getHistory();
  const hasMessages = Array.isArray(history) && history.some(m => m.role === 'user' || m.role === 'assistant');
  const wasStreaming = _deps.isStreaming();
  let savedId = '';
  if (hasMessages) {
    const activeId = await getActiveSessionId();
    const saveRes = await sendMessage({ type: 'SAVE_SESSION', id: activeId || undefined });
    savedId = saveRes?.data?.session?.id || activeId || '';
  }
  if (wasStreaming) {
    sendMessage({ type: 'REASSIGN_STREAM_SESSION', tabId: _deps.getTabId(), sessionId: savedId }).catch(() => {});
    _deps.stopWatchingStream();
    showToast(_msg('streamMovedToBackground', '进行中的回复已转入后台，完成后写入原会话'));
  }
  const res = await sendMessage({ type: 'LOAD_SESSION', id });
  // 只判内层 data.ok：外层 res.ok 恒为 true（handler 没抛），混层 && 会让失败
  // 分支永不命中——点开已删会话曾假成功（toast Loaded + 挂错 activeSessionId）。
  if (!res?.data?.ok) {
    showToast(_msg('sessionLoadFailed', 'Failed to load session'), 'error');
    return;
  }
  await setActiveSessionId(id); // 活对话现在归属于刚加载的这个会话
  await _deps.renderHistory();
  _deps.scrollToBottom(true);
  _deps.clearPendingImages();
  _deps.resumeInFlight(); // 切回：该会话若有在途流，续接渲染（STREAM_PEEK 绑定会话）
  closeSessionsDrawer();
  showToast(`${_msg('sessionLoaded', 'Loaded')}: "${name}"`, 'success');
}
