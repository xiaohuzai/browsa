...
export async function onAttachPage() {
  ...
  try {
    ...
    if (mode === 'screenshot' && ctx?.imageDataUrl) { await attachScreenshotMode(ctx, title); return; }
    if (ctx?.mode === 'pdf-pending' && ctx?.pdfBase64) { await attachPdfPending(ctx); return; }
    if (ctx?.mode === 'office-pending' && ctx?.officeBase64) { await attachOfficePending(ctx); return; }
