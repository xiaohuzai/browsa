...
// katex.bundle.js (266KB) is lazy-imported on the first sync fallback render
// instead of at module top level: the common path (worker batch) never needs
// main-thread katex at all, and a top-level import made render.js's own
// dynamic `import()` in getMermaid() a no-op cache hit — the bundle was
// parsed on every panel load either way.
let katexModule = null;
async function getKatex() {
  if (!katexModule) katexModule = import('../vendor/katex.bundle.js');
...
async function renderSync(formula, displayMode) {
  try {
...
export async function renderMathBatch(mathParts) {
  ...
  if (uncached.length <= threshold || workerFailed) {
    for (const p of uncached) results[p.origIndex] = await renderSync(p.formula, p.displayMode);
    return results;
  }
