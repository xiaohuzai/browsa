...
function _mermaidToolbar(svgWrap, source) {
  ...
  const btns = [
    ...
    { title: _msg('mermaidCopyCode', '复制代码'), html: ICONS.copy, action: (btn) => _copyText(source).then(() => { btn.textContent = '✓'; setTimeout(() => { btn.innerHTML = ICONS.copy; }, 1500); }).catch(() => {}) },
    { title: _msg('mermaidExportSvg', '导出SVG'), text: '↓', action: (btn) => _mermaidExportSvg(svgWrap).then(() => { btn.textContent = '✓'; setTimeout(() => { btn.textContent = '↓'; }, 1500); }).catch(() => {}) },
  ];
