...
export async function renderMermaid(el) {
  ...
  for (const code of [...blocks]) {
    ...
    try {
      ...
      svgWrap.innerHTML = sanitizeMermaidSvg(svg) ?? svg;
...
const _TS_TEST_RE = /(?:\*Content-)?\[(?:(\d+):)?(\d{1,2}):(\d{2})(?:(?:\s*[-,]\s*|\s+)(?:(?:\d+):)?\d{1,2}:\d{2})*\]/;
const _TS_RE = /(?:\*Content-)?\[(?:(\d+):)?(\d{1,2}):(\d{2})(?:(?:\s*[-,]\s*|\s+)(?:(?:\d+):)?\d{1,2}:\d{2})*\]/g;
...
export function makeStreamRenderer(el, { onTick, onDone } = {}) {
  ...
  renderStream.destroy = () => pacer.destroy();
  return renderStream;
}
