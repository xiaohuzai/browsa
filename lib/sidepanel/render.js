...
export function addCodeCopyButtons(root) {
  ...
  for (const pre of pres) {
    ...
    if (code && !code.dataset.highlighted) {
      ...
      if (lang && !['mermaid', 'markmap', 'diff', 'patch', 'echarts', 'smiles', 'pdb'].includes(lang)) {
        try {
...
// RCSB) vs an inline PDB payload (render directly) vs garbage (leave the
// block as text). Exported for tests.
  ...
  specs.push({
    ...
    action: _exportAction('↓', async () => {
      ...
      if (svg) {
        ...
        _downloadDataUrl(pngCanvas.toDataURL('image/png'), 'reaction.png');
      } else {
