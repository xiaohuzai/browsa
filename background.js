...
const CAPABILITY_HINTS = [
  ...
  'For chemical structures (molecules, drugs, organic compounds), output a ```smiles code block whose content is the molecule\'s SMILES string — the chat UI renders it as a 2D structure diagram natively. Example: ```smiles\nCC(=O)OC1=CC=CC=C1C(=O)O\n``` (aspirin). For chemical REACTIONS use the same ```smiles block with reaction SMILES notation reactants>agents>products (multiple species dot-separated; leave the agent section empty when none): ```smiles\nCC(=O)O.CCO>>CCOC(=O)CC.O\n``` (acetic acid + ethanol → ethyl acetate + water). Never draw molecules or reactions as ASCII art or Mermaid graphs.',
  'For protein 3D structures, output a ```pdb code block whose content is ONLY the 4-character PDB ID of a structure you are confident exists in the RCSB Protein Data Bank (e.g. ```pdb\n1UBQ\n```) — the chat UI fetches the real coordinates from RCSB and renders an interactive 3D viewer. NEVER write raw PDB ATOM coordinate lines yourself and NEVER invent a PDB ID; if you do not know a real ID for the requested protein, say so instead.',
  'For neural network / model architecture diagrams (MLP, CNN, Transformer, encoder-decoder — any stack of layers), output a ```nn code block containing JSON — the chat UI renders it as a publication-style architecture figure natively. Reserve Mermaid for graphs whose edges carry control flow or dataflow logic; a plain layer-stack architecture is ALWAYS a ```nn figure, never a Mermaid flowchart. Two forms. (1) Layer stack (default): {"layers":["Input 224×224×3","Conv2D 64 @3×3","MaxPool 2×2",{"name":"Residual block","parallel":[{"name":"Conv 3×3"},{"name":"Conv 1×1"}]},"Flatten","Dense 128","Softmax 10"],"skips":[{"from":1,"to":3,"label":"residual"}]} — each layer is a plain string (or {"name":...,"out":"output shape","kind":"input|output"}); a layer with "parallel" (2-6 objects) renders as a side-by-side branch group; "skips" draws curved skip/residual connections between layer indices (from<to). (2) Classic neuron circles, ONLY for small MLPs: {"style":"fcnn","layers":[3,5,5,2],"labels":["input","hidden","hidden","output"]} — each number is that layer\'s neuron count (values above 10 are abbreviated automatically). Keep layer names short; put tensor shapes in "out" rather than padding names.',
